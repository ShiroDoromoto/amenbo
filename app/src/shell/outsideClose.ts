 ...
 * (.setup__overlay) are ignored; a row or card in the list ([data-pane-select]) leaves the switch to its onClick
...
export function isBlankSpaceClose(target: Node | null, rightpane: HTMLElement | null): boolean {
  ...
  if (target.closest(".setup__overlay")) return false; // inside a modal: ignore
