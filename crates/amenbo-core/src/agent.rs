...
fn all_commands() -> Value {
    json!([
        ...
        cmd("automation start", "Starts an automation: checks it, copies its steps into the run so later edits cannot change what is under way, and takes a lane if one is free (queued if not). It takes nothing else — which tasks a step works on and which folder it runs in are the automation's own answers, given while it was built. An unfinished one is refused as not_ready, naming every reason: a way out with nothing after it, a required input nothing reaches, a required setting nobody answered, an agent this machine cannot start, no steps, no entry, an entry that takes no task. A model the agent does not have here is NOT among them from a terminal: knowing costs a login shell and that provider starting up, and the answers the app keeps are in the app's own process — so the GUI's launch place names it and a step started from here meets it in the pane instead. Prints the run id that pause / resume / stop take.",
            json!([{ "name": "<id>", "help": "automation id", "required": true },
                   { "name": "--json", "help": "machine-readable output" }]),
