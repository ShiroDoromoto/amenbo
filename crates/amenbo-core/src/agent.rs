...
fn all_commands() -> Value {
    json!([
        ...
        json!({ "name": "skin add", "summary": "Takes a skin file in, to `<base>/skins/<name>.yaml` under the name the file gives itself. It is read, checked and measured first. A key this build does not know, a name it keeps to itself and a value that is not text are dropped with a warning naming each; a later `skin_v`, a `themes` line that disagrees with the tables, a name that cannot be a filename, and an embedded font with no `license_text` turn the whole file away. A skin may carry one font (`font_file`: family/format/license/license_text/data as base64); it is taken only as woff2 under 2MB, and anything else about it drops the font with a warning and keeps the colours. The frame pair is held to what this build draws: `border-style` takes solid/double/none, `border-w` is put back inside 0-4px, and a `double` frame is drawn at 3px whatever was asked for (the widths that split are not one range). A colour pairing under WCAG AA is reported with its number and does NOT stop the import — an author has to be able to try work in progress. A name already held is refused with both versions named, unless --yes replaces it.",
