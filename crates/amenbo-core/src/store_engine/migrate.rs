...
/// v55: the shared documents go, and `automation_action.note` arrives (`AMB-D-952`).
///
...
///
/// **Three tables, not two.** `automation_placement_note` and `automation_note` are the pair the ops
/// read. `automation_step_note` is the one v50 laid down and v53 does not always clear: v53 skips whole
/// on a store born below v50, which genesis hands today's registry, and the registry has never carried
/// that name — so the table sits there with nothing that reads it (`AMB-T-5323` found one). It is
/// dropped here with the two it belongs to rather than left for a reader to wonder at.
///
/// The order is the `RESTRICT` clauses': a link names the document, so both link tables go before the
/// document table does — dropping a table something still points rows at is refused.
///
/// **`DROP` alone, with no `DELETE` in front of it.** `automation_step_note` points at
/// `automation_step`, which v54 has already taken away, and with foreign keys on a `DELETE` cannot
/// even be prepared against a table whose parent is missing (`no such table`). A `DROP` is prepared
/// against no parent and takes the rows with it, so it is the one statement that reaches all three.
///
...
fn take_the_shared_documents_away(ctx: &Ctx<'_>) -> Result<()> {
    ...
    tx.execute_batch(
        "DROP TABLE IF EXISTS automation_placement_note;
         DROP TABLE IF EXISTS automation_step_note;
...
mod tests {
    ...
    /// **v55 takes the shared documents away and gives an action a note** (`AMB-D-952`).
    ///
    /// Two shapes arrive here. A store that reached v53 with documents on it carries all three tables
    /// — the pair the ops read, and the `automation_step_note` v50 laid down — with a document's body
    /// copied into the word index. A store born below v50 was handed today's registry by genesis,
    /// so v53 skipped it whole and it arrives carrying `automation_step_note` alone, pointing at a
    /// table v54 has already taken away. Both come out with none of the three, with nothing left of
    /// the documents in `search_doc`, and with the column the build screen draws what an action is
    /// for from.
    #[test]
    fn the_shared_documents_and_their_leftover_table_go() {
        ...
        let conn = engine.conn();
        let one = |sql: &str| -> i64 { conn.query_row(sql, [], |r| r.get(0)).unwrap() };
        for gone in ["automation_note", "automation_placement_note", "automation_step_note"] {
            assert_eq!(
                one(&format!(
        ...
        // The other shape: v53 skipped it whole, so `automation_step_note` is the one of the three it
        // arrives with — and its `REFERENCES` names `automation_step`, which v54 took away.
        let dir = scratch("shared-documents-baseline");
        let engine = store_at(&dir, OLDEST_FROZEN_VERSION);
        run(&engine, &dir, STEPS, &mut crate::progress::ignore).unwrap();
        let conn = engine.conn();
        let one = |sql: &str| -> i64 { conn.query_row(sql, [], |r| r.get(0)).unwrap() };
        assert_eq!(
            one("SELECT COUNT(*) FROM sqlite_master \
                 WHERE type = 'table' AND name = 'automation_step_note'"),
            0,
            "the table v50 lays down and v53 never reaches goes here",
        );
        std::fs::remove_dir_all(&dir).ok();
    }
