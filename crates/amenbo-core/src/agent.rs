...
fn capabilities() -> Value {
    let caps = vec![
        ...
        cap(
            "Build an automation — the library of actions, the steps in them, the placements on a picture, the documents they share and the settings each answers",
            &[
...
fn all_commands() -> Value {
    json!([
        ...
        cmd("automation add", "Creates an automation: a name, what it is for, and the preamble prepended to every step's launch. Left out, --preamble puts the standing operating rules there — what every step is told before its own prompt — and an empty string is how an automation carries none. It starts with nothing placed on it and no entry, which is not refused here: the launch check is where an unfinished one is caught. Prints the id the place and note commands take.",
        ...
        cmd("automation rm", "Deletes an automation with every placement, edge, wire and shared document built onto it, and the answers written on each placement. The library actions those placements stood on are left where they are. It rides one transaction, so there is no half-deleted picture. Confirms unless --yes.",
            json!([{ "name": "<id>", "help": "automation id", "required": true },
        ...
        cmd("automation show", "One automation in full, which is the only way to read a definition back from a terminal. It prints the automation's own notes and preamble, then every placement with what it runs under — the action standing there, what it takes in, what it is answered with, each way out with what leaving through it hands on, what happens after each one is taken, and what is wired from it to a later placement — then the documents its placements share. What is inside an action is `automation action-show`'s. Document bodies are printed whole: a definition is read back to check what was built, and a snippet would send the reader to a screen to finish the sentence. An edge hanging on a way out the action no longer declares is written out as such, that being why a picture stops walking.",
            json!([{ "name": "<id>", "help": "automation id", "required": true },
                   { "name": "--json", "help": "machine-readable output" }]),
        ...
        cmd("automation step-add", "Adds a step to a library action — one step is one terminal, and it carries its own prompt. It is born with the unnamed way out and the error one (`*`). --work-dir names the setting or the input the working folder is taken from: a name, not a path, and the name is one the action declares. Prints the id the action's own edge and wire commands take.",
            ...
            json!(["amenbo automation step-add 7 --name \"Review\" --prompt - --agent claude",
                   "amenbo automation step-add 7 --name \"Fix\" --prompt - --agent claude --work-dir folder"])),
        cmd("automation step-update", "Changes a step. Only the fields given change. Rewriting the prompt reaches every placement of the action holding it, which is what the library is for; a run already under way is not reached, its copy having been taken at the launch.",
        ...
        cmd("automation wire-add", "Joins what one way out hands on to what a later box takes in. Both ends are named rather than keyed: one action placed twice on an automation gives two placements whose ports carry the same names, so only the box, the way out and the port name together say which is meant. --from is the same <box>:<way out> token the edge takes, and --in-action picks the same picture it picks there. The two ends are refused unless they carry the same kind of thing.",
            json!([{ "name": "--from <box>:<way out>", "help": "where it comes from (`4:` is the unnamed way out)", "required": true },
                   { "name": "--in-action", "help": "draw it inside a library action, between its steps" },
                   { "name": "--from-port <name>", "help": "the output's name on that way out", "required": true },
                   { "name": "--to <id>", "help": "the box that takes it in", "required": true },
                   { "name": "--to-port <name>", "help": "the input's name on that box", "required": true }]),
        ...
        cmd("automation note-add", "Writes a document the placements of one automation share. Long is fine here — this is where the material a prompt would otherwise repeat belongs, which is what keeps the preamble short. Which placements are handed it is `automation note-link`'s to say.",
            json!([{ "name": "<automation>", "help": "automation id", "required": true },
        ...
        cmd("automation note-rm", "Deletes a shared document with the links that hand it to placements. Confirms unless --yes.",
            json!([{ "name": "<id>", "help": "document id", "required": true },
                   { "name": "--yes/-y", "help": "skip the confirmation" }]),
            json!(["amenbo automation note-rm 71 --yes"])),
        cmd("automation note-link", "Hands a shared document to a placement: it is read out to every step opened there, alongside the preamble and that step's own prompt. A document may go to several placements, and a placement may be handed several.",
            json!([{ "name": "<placement>", "help": "placement id", "required": true },
        ...
        cmd("automation note-unlink", "Stops handing a shared document to a placement. The document itself stays where it is.",
            json!([{ "name": "<placement>", "help": "placement id", "required": true },
                   { "name": "<note>", "help": "document id", "required": true }]),
