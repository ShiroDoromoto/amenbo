...
const UI: Record<Lang, Record<string, string>> = {
  ja: {
    ...
    "newproj.copyStatus": "amenbo status をコピー", "newproj.copied": "✓ コピーしました",
    ...
    "dec.accept": "採択", "dec.reject": "却下", "dec.reopen": "編集に戻す",
    ...
    "dec.edge.supersedeAcceptsConfirm": "{target} を置き換えます。置き換えるからには決まっているので、この議論中の決定は採択済みになり、本文は凍結されます。よろしいですか？",
    ...
    "dec.filterAll": "すべて",
    "dec.searchPh": "タイトル・本文・AMB-D-<n> を検索",
    "dec.sort": "並び替え",
    ...
    "detail.membership": "所属", "detail.addMembership": "所属", "detail.none": "なし",
    ...
    "settings.doctorFix": "修復する",
    "settings.doctorFixing": "修復中…",
    "settings.doctorFixNote": "「修復する」がまとめて直すのは、孤児コメントの削除・未参照の添付ファイルの回収・残骸のフォルダ紐付けの掃除です。紐付けフォルダの問題は、直し方が一つに決まる行にだけボタンが付きます（決まらない行は、プロジェクト設定 > フォルダで紐付け先を選んでください）。",
    ...
    "hooks.title": "コミット時に lint を走らせますか？",
    "hooks.why": "`{cmd}` が、amenbo の参照（AMB-T-… など）をコミットへ持ち出す前に弾きます。git のフックとして設置します。",
    "hooks.where": "{project} — {dir}",
    "hooks.willWrite": "設置するもの:",
    "hooks.foreign": "こちらは amenbo のものではないので触りません。自分でこの1行を足してください:",
    "hooks.install": "設置する",
    "hooks.notNow": "今はしない",
    "hooks.never": "二度と聞かない",
    "hooks.hint": "「今はしない」は次回また聞きます。「二度と聞かない」の後も `{cmd} hooks install` でいつでも設置できます。",
    "hookSetup.title": "lint がコミットに掛かっていません（セットアップ未完了）",
    "hookSetup.where": "{project} — {dir}",
    "hookSetup.unwired": "{slots}: フックがありません。`{cmd}` で設置できます。",
    "hookSetup.foreign": "{slot}: amenbo のものではないフックがあるので触りません。この1行を自分で足してください: {line}",
    ...
    "update.hint": "新しい amenbo がこのストアを使っています。最新インストーラ（GUI+CLI 同梱）で更新してください（自動更新はしません）。",
    "update.open": "インストーラを開く",
    "update.dismiss": "閉じる",
  ...
  en: {
    ...
    "newproj.copyStatus": "Copy amenbo status", "newproj.copied": "✓ Copied",
    ...
    "dec.accept": "Accept", "dec.reject": "Reject", "dec.reopen": "Reopen for editing",
    ...
    "dec.edge.supersedeAcceptsConfirm": "Supersede {target}. Superseding means you have decided, so this proposed decision becomes accepted and its body freezes. Continue?",
    ...
    "dec.filterAll": "All",
    "dec.searchPh": "Search title / body / AMB-D-<n>",
    "dec.sort": "Sort",
    ...
    "detail.membership": "In", "detail.addMembership": "Add", "detail.none": "None",
    ...
    "settings.doctorFix": "Repair",
    "settings.doctorFixing": "Repairing…",
    "settings.doctorFixNote": "“Repair” covers deleting orphan comments, reclaiming unreferenced attachment files, and dropping leftover folder bindings, all at once. Bound-folder problems get their own button on the lines whose fix is unambiguous (for the rest, pick the project in Project settings > Folders).",
    ...
    // lint hook consent: the question amenbo asks before writing into .git/hooks
    "hooks.title": "Lint your commits on the way out?",
    "hooks.why": "`{cmd}` keeps amenbo's own refs (AMB-T-…) out of your commits. It runs as a git hook.",
    "hooks.where": "{project} — {dir}",
    "hooks.willWrite": "Will be installed:",
    "hooks.foreign": "These are not amenbo's and will be left alone. Add this line yourself:",
    "hooks.install": "Install",
    "hooks.notNow": "Not now",
    "hooks.never": "Never ask again",
    "hooks.hint": "“Not now” asks again next time. After “never”, `{cmd} hooks install` still works whenever you want it.",
    "hookSetup.title": "The lint is not running on your commits (setup unfinished)",
    "hookSetup.where": "{project} — {dir}",
    "hookSetup.unwired": "{slots}: no hook there. `{cmd}` installs it.",
    "hookSetup.foreign": "{slot}: held by a hook that is not amenbo's, so it is left alone. Add this line yourself: {line}",
    ...
    "update.hint": "A newer amenbo has used this store. Get the latest installer (GUI + CLI together) to update — amenbo does not self-update.",
    "update.open": "Open installer",
