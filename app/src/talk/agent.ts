...
export async function mountAgentFrame(
  ...
  const look = async () => {
    clear();
    // A step's terminal is its run's pane's and nobody else's, so it is not counted among the ones a
    // pane may take up by being the only one (`crate::pty::open_step`).
  ...
  const open = (choice: string | null, take: PaneStart = { adopt: false }) => {
    ...
    // A run's place asks every terminal started in it for a session of its own. `take` is `start`
    // itself on the one road that takes a terminal up, and its own answer wins there for the same
    // reason it does for the folder. It is put on only where there is one to put, so an ordinary
    // pane is started with exactly what it was started with before.
