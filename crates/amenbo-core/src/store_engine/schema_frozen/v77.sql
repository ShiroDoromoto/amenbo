...
CREATE TABLE IF NOT EXISTS attachment (
    id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
    target_type TEXT NOT NULL DEFAULT '' CHECK(target_type IN ('', 'task', 'decision', 'task_comment', 'decision_comment', 'automation_run_step', 'automation_run')),
...
CREATE TABLE IF NOT EXISTS automation_action_step (
    ...
    show_notes BOOLEAN NOT NULL DEFAULT 0 CHECK(show_notes IN (0, 1)),
    show_decisions BOOLEAN NOT NULL DEFAULT 0 CHECK(show_decisions IN (0, 1)),
    show_comments BOOLEAN NOT NULL DEFAULT 0 CHECK(show_comments IN (0, 1)),
    order_key TEXT NOT NULL DEFAULT '',
    created_at TEXT NOT NULL DEFAULT '' CHECK(created_at = '' OR created_at GLOB '[0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9]T[0-9][0-9]:[0-9][0-9]:[0-9][0-9]Z'),
    updated_at TEXT NOT NULL DEFAULT '' CHECK(updated_at = '' OR updated_at GLOB '[0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9]T[0-9][0-9]:[0-9][0-9]:[0-9][0-9]Z')
);
...
CREATE TABLE IF NOT EXISTS automation_run_step (
    ...
    report_withheld BOOLEAN NOT NULL DEFAULT 0 CHECK(report_withheld IN (0, 1)),
