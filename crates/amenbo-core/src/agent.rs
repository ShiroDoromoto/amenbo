...
const JA_PHRASEBOOK: &[(&str, &str)] = &[
    ...
    ("Shows a dimension: name, its readable key (slug), notes, kind (single-select, ordered, time-axis, show-on-card, required, and the one side it is narrowed to, if it is), and its values with their own keys.", "次元を表示します: 名前、読める鍵（slug）、メモ、種類（単一選択、順序付き、時間軸、カードに出す、必須、そして片側に絞ってあればその側）、そして値とその鍵。"),
    ...
    ("Updates a dimension's name, notes, value ordering, time-axis role, whether it goes on the task card, whether it must be answered, which side of the store it classifies, and/or its readable key. Only the given fields change.", "次元の名前・メモ・値の順序・時間軸の役割・タスクカードに出すかどうか・答えを必須にするかどうか・ストアのどちら側に効くか・読める鍵を更新します。指定したフィールドだけが変わります。"),
    ...
    ("Assigns a task or a decision a value of a dimension — one axis and one set of values, whichever of the two is being filed. An axis is single-select, so the prior value on that axis is replaced. The target must say which kind it is: a bare number is refused, tasks and decisions numbering independently.", "タスクにも決定記録にも次元の値を割り当てます。軸も値も両者で同じものを使います。軸は単一選択なので、その軸の以前の値は置き換わります。対象はどちらの種類かを名乗る必要があります: タスクと決定記録は別々に採番されるため、裸の数字は拒否されます。"),
    ...
    ("classify at creation, resolving names as dimension set does; repeatable for different axes (an axis is single-select, so naming one twice is refused, and so is an axis that does not classify decisions). It saves the create→dimension set round trip, and a required axis filled here is one `decision accept` will not turn the acceptance away for", "作成時に分類軸の値を割り当てます。名前の解決は dimension set と同じで、軸ごとに繰り返し指定できます（軸は単一選択なので同じ軸を2度指定すると拒否。決定を分類しない軸も拒否）。作成→dimension set の往復を省き、必須の軸をここで埋めておけば decision accept が採択を断りません"),
...
fn all_commands() -> Value {
    json!([
        ...
        json!({ "name": "dimension show", "summary": "Shows a dimension: name, its readable key (slug), notes, kind (single-select, ordered, time-axis, show-on-card, required, and the one side it is narrowed to, if it is), and its values with their own keys.",
            "args": [{ "name": "id", "required": true, "help": "dimension id, slug or name" }],
            "flags": [{ "name": "--json", "help": "machine-readable output" }],
            "examples": ["amenbo dimension show Category --json"] }),
        json!({ "name": "dimension update", "summary": "Updates a dimension's name, notes, value ordering, time-axis role, whether it goes on the task card, whether it must be answered, which side of the store it classifies, and/or its readable key. Only the given fields change.",
            "args": [{ "name": "id", "required": true, "help": "dimension id, slug or name" }],
        ...
        json!({ "name": "dimension set", "summary": "Assigns a task or a decision a value of a dimension — one axis and one set of values, whichever of the two is being filed. An axis is single-select, so the prior value on that axis is replaced. The target must say which kind it is: a bare number is refused, tasks and decisions numbering independently.",
        ...
        json!({ "name": "decision add", "summary": "Records a new decision (proposed) — an append-only \"why we chose X\". A decision is a Task sibling (project-scoped), NOT a task: it has no mailbox workflow and never appears in task lists. Decisions have their own device-global number space, shown as AMB-D-N (tasks are AMB-T-N); the kind code keeps AMB-D-<n> / AMB-T-<n> unambiguous. The body should be the conclusion + rationale (compress; do not paste raw discussion, and keep PII out). The project defaults to the bound project. Classify it here with --dim: an axis the project marked required is read at `decision accept`, so one left empty is what turns the acceptance away — filling it at creation is what saves that round trip. The response names any required axis still blank (`unmet_required_dimensions` in --json), since a decision has no second stage to be turned away at and the one who accepts it is not the one who wrote it.",
            "args": [], "flags": [{ "name": "--title <str>", "required": true, "help": "decision title" },
                                   ...
                                   { "name": "--dim <axis>=<value>", "help": "classify at creation, resolving names as dimension set does; repeatable for different axes (an axis is single-select, so naming one twice is refused, and so is an axis that does not classify decisions). It saves the create→dimension set round trip, and a required axis filled here is one `decision accept` will not turn the acceptance away for" },
