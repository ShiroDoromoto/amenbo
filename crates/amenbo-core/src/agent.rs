...
fn spec_as_authored() -> Value {
    json!({
        ...
        "agentCycle": [
            ...
            "0. intake: if the work in front of you came from the human in this session and is not on the backlog yet, put it there first — `amenbo task add ... --to me-ai` when you are the one continuing — then join at step 2 (reserve) with the id it hands back. Work you pulled off the backlog yourself is already recorded, so it needs no intake: enter at step 1.",
