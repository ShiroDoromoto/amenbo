...
/// inside the record that holds them and have no destination to name. A kind with nowhere to lead is
/// left as plain text rather than dressed as a link that would go nowhere.
