...
const JA_PHRASEBOOK: &[(&str, &str)] = &[
    ...
    ("Shows a dimension: name, notes, kind (single-select, ordered, time-axis, show-on-card, required), and its values.", "次元を表示します: 名前、メモ、種類（単一選択、順序付き、時間軸、カードに出す、必須）、そして値。"),
    ("Updates a dimension's name, notes, value ordering, time-axis role, whether it goes on the task card, and/or whether it must be answered. Only the given fields change.", "次元の名前・メモ・値の順序・時間軸の役割・タスクカードに出すかどうか・答えを必須にするかどうかを更新します。指定したフィールドだけが変わります。"),
...
fn all_commands() -> Value {
    json!([
        ...
        json!({ "name": "dimension show", "summary": "Shows a dimension: name, notes, kind (single-select, ordered, time-axis, show-on-card, required), and its values.",
            "args": [{ "name": "id", "required": true, "help": "dimension id or name" }],
            "flags": [{ "name": "--json", "help": "machine-readable output" }],
            "examples": ["amenbo dimension show Category --json"] }),
        json!({ "name": "dimension update", "summary": "Updates a dimension's name, notes, value ordering, time-axis role, whether it goes on the task card, and/or whether it must be answered. Only the given fields change.",
            "args": [{ "name": "id", "required": true, "help": "dimension id or name" }],
