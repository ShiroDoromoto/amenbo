...
fn capabilities() -> Value {
    let caps = vec![
        ...
        cap(
            "Put a plugin from the catalog on this machine, see what is installed and what the catalog has moved past, open or close each one's gate (install ≠ enable, machine-wide or for this project), and remove one with everything it left behind",
            &[
                "plugin list",
                "plugin install",
                "plugin update",
...
fn all_commands() -> Value {
    json!([
        ...
        json!({ "name": "plugin update", "summary": "Reports which installed plugins the catalog holds a different build of. Detection is the catalog amenbo already fetches whole laid beside the manifest that sits next to each installed binary — no central server, no per-plugin request. A manifest carries no version number, so what is compared is the asset's checksum: the digest of the exact bytes, and so the build's identity — two entries with the same digest are the same executable however the description around them was rewritten. It therefore reports different, not newer: a catalog that rolls an entry back offers that older build, because the catalog is the authority on what is published. A plugin the catalog does not list is passed over rather than reported (installed by hand, or delisted). Cheap on purpose: with nothing installed no catalog is read at all, and otherwise a cached catalog younger than an hour answers with no request — which is what lets a check ride along with something you were doing anyway instead of needing a timer. Applying an update is a separate act, so --check is required today.",
            "flags": [{ "name": "--check", "help": "report what has an update without applying anything (required)" }, { "name": "--json", "help": "machine-readable output" }],
            "examples": ["amenbo plugin update --check", "amenbo plugin update --check --json"] }),
