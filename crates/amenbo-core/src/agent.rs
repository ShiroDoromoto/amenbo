...
const JA_PHRASEBOOK: &[(&str, &str)] = &[
    ...
    ("Updates amenbo. By default opens this OS's one-piece installer (GUI + CLI) — resolved from the published latest.json, falling back to the releases page — in your browser. Because you typed it, the lookup asks upstream rather than answering from the detection cache, so it never reports on an entry up to an hour old (offline it still falls back to the last one it had). `--apply` self-updates the standalone CLI in place instead: it downloads the new CLI over TLS and swaps this binary (no installer, no elevation), keeping the replaced binary beside it; a GUI-managed CLI is updated from the desktop app, not here. `--rollback` undoes the last `--apply` offline, restoring that kept binary. Applying is always your explicit call — amenbo never updates in the background.", "amenbo を更新します。既定ではこの OS 向けの一体型インストーラ（GUI+CLI 同梱）を——公開 latest.json から解決し、無ければリリースページへフォールバック——既定ブラウザで開きます。打ったのは人なので、照会は検知キャッシュで答えずに上流へ取りに行きます——最大1時間前の控えで答えることはありません（オフラインでは従来どおり最後の控えへ落ちます）。`--apply` は代わりに単独 CLI をその場で自己更新します：新しい CLI を TLS で取得してこのバイナリを置換し（インストーラ不要・昇格不要）、置換前のバイナリを隣に残します；GUI 管理下の CLI はここではなくデスクトップアプリから更新されます。`--rollback` は直前の `--apply` をオフラインで取り消し、残しておいたバイナリを戻します。適用は常に利用者の明示操作で——amenbo が背後で更新することはありません。"),
    ...
    ("Lists decisions (status:proposed|accepted|rejected, superseded:yes|no — the edge itself, so superseded:yes lists the decisions another decision draws a supersedes edge at — text: over title+body+comment bodies+attachment names, project:, decided_before:/decided_after: over the day a decision was accepted (YYYY-MM-DD, or today/-30d; both ends inclusive; a decision that was never accepted has no such day and matches neither)). To ask which policies were settled by a date, compose this filter with superseded: — there is no separate as-of switch, and the composition recovers neither status transitions nor deleted decisions. Sort by decided/created/number/title/status (prefix - for descending; default -created). --limit/--offset page (JSON carries total_matched = the count before paging, count = this page). --with-body adds each decision's body to the rows — a projection that composes with --filter/--limit/--offset (narrow by keywords/status and page; it does not dump the whole corpus).", "決定記録を一覧します（status:proposed|accepted|rejected、superseded:yes|no ── エッジそのもので引きます。superseded:yes ＝ 別の決定が supersedes エッジを張っている決定、text: はタイトル＋本文＋コメント本文＋添付の名前が対象、project:、decided_before:/decided_after: は採択された日で絞ります〔YYYY-MM-DD または today/-30d・両端を含む。採択されていない決定はその日を持たず、どちらにも一致しません〕）。「T までに決まっていた方針」は、このフィルタと superseded: の合成で得ます ── 専用の as-of スイッチはありません。合成でもステータス遷移の履歴や削除された決定は辿れません。並び替えは decided/created/number/title/status（先頭 - で降順・既定 -created）。--limit/--offset はページングします（JSON は total_matched = ページング前の件数、count = このページの件数を持ちます）。--with-body は各決定の本文を行に足します ── --filter/--limit/--offset と合成できる射影です（キーワード/状態で絞ってページングする。コーパス全体を吐き出しはしません）。"),
    ("Shows a decision: body, status, the supersession chain (both directions), the premises it builds on (read those first — a premise another decision has overturned is flagged, because this decision then stands on rotten ground) and the decisions that build on it (the impact radius: overturn this one and they want revisiting), and its linked tasks. Its timeline comes with it, in the same shape task show gives its own: the count of what has been said, the newest three previewed a line each, and the way to the rest — which is where the reason an acceptance or a rejection was given lives, since --reason writes one as a comment (--json returns every comment whole, under `comments`).", "決定記録を表示します: 本文、状態、置換チェーン（両方向）、前提にしている決定（先に読むべき決定。別の決定に覆された前提には印が付きます ── その決定は腐った前提の上に立っています）、この決定を前提にする決定（影響半径: これを覆すなら見直しが要る決定）、関連タスク。 コメント面も task show と同じ形で併せて出ます: 何件付いているか、新しい順に3件のプレビュー、残りへの導線。採択・却下の理由は --reason がコメントとして書くので、理由が読めるのはここです（--json は全件を `comments` で丸ごと返します）。"),
    ...
    ("machine-readable output — { query, count, total_matched, hits[face,kind,ref,title,comment,at,snippet] }", "機械可読な出力 ── { query, count, total_matched, hits[face,kind,ref,title,comment,at,snippet] }"),
    ...
    json!({
        ...
        "filterGrammar": {
            ...
            "keys": {
                ...
                "text": ["<substring match over title + notes + comment bodies + labels + attachment names (folded: width, case, hiragana=katakana)>"],
    ...
    json!([
        ...
        json!({ "name": "search", "summary": "Finds where words are written, across tasks, decisions and the comments on both — plus the labels a task is filed under and the names of what is attached. Answers with one line per PLACE, not per record: the face the words landed on (title / body / comment / label / attachment), the record it belongs to, the comment to open where there is one, and a short excerpt. Words are ANDed and match as substrings, with no word boundaries (so a search for part of a compound word finds it); full-width, case and kana spellings are folded together. Every word has to land somewhere on the record — not all on one face — and each face carrying one is a line, which is what makes the answer 'here is where each of your words is written'. A word written as a ref (AMB-T-<n> / AMB-D-<n>) pins that record to the top, so holding a number takes the same command as holding a phrase. Default order is the face first (a name outranks a paragraph, and that a remark), newest within it. This read has a ceiling of its own — a hit carries an excerpt — so with no --limit it returns the first 20 and reports the total.",
            ...
            "flags": [{ "name": "--filter <expr>", "help": "narrow structurally, in the grammar task list uses (see filterGrammar). That is task vocabulary, so a search carrying one is a search of tasks" },
                      ...
                      { "name": "--json", "help": "machine-readable output — { query, count, total_matched, hits[face,kind,ref,title,comment,at,snippet] }" }],
            "examples": ["amenbo search plugin distribution --json", "amenbo search AMB-T-<n> --json", "amenbo search rollout --kind decision --json", "amenbo search plugin --filter \"status:todo\" --limit 5 --json"] }),
        ...
        json!({ "name": "task show", "summary": "Shows task details — project, classification (dimensions: the axis=value pairs it is filed under, absent when it is filed under none), blockers (blocked_by) and dependents (blocks: what finishing this task would unblock). It carries the timeline too: the count of what has been said on the task, the newest three previewed a line each (cut where they run long), and the way to the rest — --json returns every comment whole, under `comments`.",
        ...
        cmd("decision list", "Lists decisions (status:proposed|accepted|rejected, superseded:yes|no — the edge itself, so superseded:yes lists the decisions another decision draws a supersedes edge at — text: over title+body+comment bodies+attachment names, project:, decided_before:/decided_after: over the day a decision was accepted (YYYY-MM-DD, or today/-30d; both ends inclusive; a decision that was never accepted has no such day and matches neither)). To ask which policies were settled by a date, compose this filter with superseded: — there is no separate as-of switch, and the composition recovers neither status transitions nor deleted decisions. Sort by decided/created/number/title/status (prefix - for descending; default -created). --limit/--offset page (JSON carries total_matched = the count before paging, count = this page). --with-body adds each decision's body to the rows — a projection that composes with --filter/--limit/--offset (narrow by keywords/status and page; it does not dump the whole corpus).",
