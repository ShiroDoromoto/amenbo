...
const UI: Record<Lang, Record<string, string>> = {
  ja: {
    ...
    "hookSetup.unwired": "{slots}: lint がまだ掛かっていません。`{cmd}` で設置できます（他ツールのフックがあれば残したまま隣に追記します）。",
  ...
  en: {
    ...
    "hookSetup.where": "{project} — {dir}",
    "hookSetup.unwired": "{slots}: the lint is not running there. `{cmd}` installs it (keeping any other tool's hook and adding the block beside it).",
