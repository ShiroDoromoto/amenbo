...
const JA_PHRASEBOOK: &[(&str, &str)] = &[
    ...
    ("Put a plugin from the catalog on this machine, see what is installed, open or close each one's gate (install ≠ enable, machine-wide or for this project), and remove one with everything it left behind", "目録のプラグインをこのマシンに入れ、インストール済みのものを見て、それぞれのゲートを（マシン全体、またはこのプロジェクトだけで）開閉し（install ≠ enable）、残したものごと撤去する"),
    ("Lists the plugins installed on this machine — name, description and the official badge — beside whose gate is open. The two facts sit together because installing a plugin never runs it: an installed plugin that fires nothing is the ordinary state, not a fault. The gate shown is the effective one: this project's override where it declares one, the machine-wide answer otherwise, because 'does it fire here' is the question being asked; --json carries both, plus the override itself, so a reader sees which tier decided. Reads only the app-data plugins/ directory and this machine's enable state — no network, no catalog fetch — so it answers the same offline. A directory it cannot read as an install is skipped rather than allowed to hide the rest. --json adds each plugin's subscribed events and the path of the executable amenbo would run.", "このマシンにインストール済みのプラグインを——名前・説明・公式バッジ——ゲートが開いているかと並べて一覧します。2つの事実を並べるのは、インストールしただけではプラグインは決して実行されないからです：インストール済みで何も発火しないのは通常の状態であって、異常ではありません。示すゲートは実効の答えです：このプロジェクトが上書きを持っていればそれを、なければマシン全体の答えを——問われているのは「ここで発火するか」だからです。--json は両方と上書きそのものを返すので、どちらの層が決めたかが分かります。読むのは app-data の plugins/ ディレクトリとこのマシンの有効化状態だけ——ネットワークも目録の取得も使わない——ので、オフラインでも同じ答えを返します。インストールとして読めないディレクトリは、残りを覆い隠さないよう読み飛ばします。--json では各プラグインの購読イベントと、amenbo が実行する実行ファイルのパスも返します。"),
    ...
    ("Enables an installed plugin: records the one-time consent to run its code, and opens the gate it fires through. Installing puts a plugin on disk and nothing more — this is the step that lets it run, and it is asked once: a later disable keeps the consent, so re-enabling never asks again. --scope picks which gate opens: machine (the default, everywhere) or project (this project alone, leaving the machine gate shut). Consent is the device's answer either way, so a project-scoped enable records it too — and an override can never grant it, which is what keeps an exported override from firing on a machine that never consented. Fail-closed on the settings the plugin's author marked required — while one is empty the enable is refused and the empty fields are named; fill them with `plugin config set` and enable again. amenbo checks only that a value is present, at the tier being opened (a machine-wide enable reads the machine defaults; a project-scoped one counts that project's overrides on top); whether the value is *meaningful* is the plugin author's to judge at run time. Fail-closed on compatibility too: a plugin whose manifest reads a different event-payload contract than this amenbo speaks, or needs an amenbo newer than the one running, is refused with both versions named — update amenbo (or the plugin) rather than run one against a payload it cannot read.", "インストール済みのプラグインを有効化します：そのコードを実行することへの一度きりの同意を記録し、発火するゲートを開きます。インストールはプラグインをディスクに置くだけで、実行を許すのはこの手順です。同意を尋ねるのは一度だけ——後で disable しても同意は残るので、再度の enable で尋ね直しません。どのゲートを開くかは --scope が選びます：machine（既定・どこでも）か project（このプロジェクトだけ・マシンのゲートは閉じたまま）。同意はどちらでも端末の答えなので、project 指定の有効化でも記録します——そして上書きが同意を与えることは決してありません。これが、書き出された上書きが同意していない端末で発火しないための要です。作者が required と印した設定に対しては fail-closed です——1つでも空なら enable は拒否され、空のフィールド名が示されます。`plugin config set` で埋めてから enable し直してください。amenbo が見るのは値が入っているかどうかだけで、見る層は開こうとしている層です（マシン全体の有効化はマシン既定を、project 指定ならそのプロジェクトの上書きも数えます）。値が意味として妥当かどうかは、実行時に作者が判断することです。互換宣言に対しても fail-closed です：manifest が読むイベントペイロード規約がこの amenbo の話す規約と違う場合、または必要とする amenbo の版が実行中の版より新しい場合、双方の版を示して拒否します——読めないペイロードを渡して走らせるのではなく、amenbo（またはプラグイン）を更新してください。"),
    ("the installed plugin's name", "インストール済みプラグインの名前"),
    ("Closes a plugin's gate: it stops firing, while staying installed and staying consented — so enabling it again later asks for nothing. --scope picks which gate closes: machine (the default) or project, which writes this project's veto and stands even while the machine gate is open (to follow the machine answer again, `plugin inherit`). Deliberately does not require the plugin to still read as installed: this is how a plugin is stopped, and a half-broken install is exactly when stopping it matters most. Disabling one that is already off changes nothing and says so.", "プラグインのゲートを閉じます：発火は止まりますが、インストール済みのまま・同意も保持したままです——後で有効化し直すときに何も尋ねられません。どのゲートを閉じるかは --scope が選びます：machine（既定）か project——後者はこのプロジェクトの拒否を書き、マシンのゲートが開いていても効き続けます（マシンの答えに従い直すには `plugin inherit`）。プラグインがインストール済みとして読めることを、あえて要求しません：これはプラグインを止める手段であり、壊れかけたインストールこそ止めたい場面だからです。すでに無効なものを disable しても何も変わらず、その旨を伝えます。"),
    ...
    ("Drops this project's gate override, so the machine-wide answer decides here again. It is the third state, and the one `disable --scope project` is not: a project veto keeps the plugin off here however the machine gate moves, while inheriting means 'whatever the machine says'. Only a project can hold an override, so there is no --scope; with none to drop it reports so and changes nothing.", "このプロジェクトのゲート上書きを外し、マシン全体の答えがここでも決めるようにします。これは第3の状態であり、`disable --scope project` とは別物です：プロジェクトの拒否はマシンのゲートがどう動いてもここでは止めたままにしますが、継承は「マシンの答えのまま」を意味します。上書きを持てるのはプロジェクトだけなので --scope はありません。外すものが無ければその旨を伝え、何も変えません。"),
    ("which gate to open (default machine)", "開くゲート（既定は machine）"),
    ("which gate to close (default machine)", "閉じるゲート（既定は machine）"),
...
fn capabilities() -> Value {
    let caps = vec![
        ...
        cap(
            "Put a plugin from the catalog on this machine, see what is installed, open or close each one's gate (install ≠ enable, machine-wide or for this project), and remove one with everything it left behind",
            &["plugin list", "plugin install", "plugin enable", "plugin disable", "plugin inherit", "plugin uninstall"],
        ),
        cap(
...
fn all_commands() -> Value {
    json!([
        ...
        json!({ "name": "plugin enable", "summary": "Enables an installed plugin: records the one-time consent to run its code, and opens the gate it fires through. Installing puts a plugin on disk and nothing more — this is the step that lets it run, and it is asked once: a later disable keeps the consent, so re-enabling never asks again. --scope picks which gate opens: machine (the default, everywhere) or project (this project alone, leaving the machine gate shut). Consent is the device's answer either way, so a project-scoped enable records it too — and an override can never grant it, which is what keeps an exported override from firing on a machine that never consented. Fail-closed on the settings the plugin's author marked required — while one is empty the enable is refused and the empty fields are named; fill them with `plugin config set` and enable again. amenbo checks only that a value is present, at the tier being opened (a machine-wide enable reads the machine defaults; a project-scoped one counts that project's overrides on top); whether the value is *meaningful* is the plugin author's to judge at run time. Fail-closed on compatibility too: a plugin whose manifest reads a different event-payload contract than this amenbo speaks, or needs an amenbo newer than the one running, is refused with both versions named — update amenbo (or the plugin) rather than run one against a payload it cannot read.",
            ...
            "flags": [{ "name": "--scope <machine|project>", "help": "which gate to open (default machine)" }, { "name": "--json", "help": "machine-readable output" }],
            "examples": ["amenbo plugin enable worktree", "amenbo plugin enable slack --scope project"] }),
        json!({ "name": "plugin disable", "summary": "Closes a plugin's gate: it stops firing, while staying installed and staying consented — so enabling it again later asks for nothing. --scope picks which gate closes: machine (the default) or project, which writes this project's veto and stands even while the machine gate is open (to follow the machine answer again, `plugin inherit`). Deliberately does not require the plugin to still read as installed: this is how a plugin is stopped, and a half-broken install is exactly when stopping it matters most. Disabling one that is already off changes nothing and says so.",
            "args": [{ "name": "name", "required": true, "help": "the plugin's name" }],
            "flags": [{ "name": "--scope <machine|project>", "help": "which gate to close (default machine)" }, { "name": "--json", "help": "machine-readable output" }],
            "examples": ["amenbo plugin disable worktree", "amenbo plugin disable slack --scope project"] }),
        json!({ "name": "plugin inherit", "summary": "Drops this project's gate override, so the machine-wide answer decides here again. It is the third state, and the one `disable --scope project` is not: a project veto keeps the plugin off here however the machine gate moves, while inheriting means 'whatever the machine says'. Only a project can hold an override, so there is no --scope; with none to drop it reports so and changes nothing.",
            "args": [{ "name": "name", "required": true, "help": "the plugin's name" }],
            "flags": [{ "name": "--json", "help": "machine-readable output" }],
            "examples": ["amenbo plugin inherit slack"] }),
