...
fn all_commands() -> Value {
    json!([
        ...
        cmd("automation stop", "Stops a run: hands the task it was working back to todo, and leaves a comment on that task saying how far it got. Unlike pause it does not wait for the step under way — use it where a pause will not land.",
            json!([{ "name": "<run>", "help": "run id", "required": true },
                   { "name": "--json", "help": "machine-readable output" }]),
