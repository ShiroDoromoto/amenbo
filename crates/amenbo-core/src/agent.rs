...
fn capabilities() -> Value {
    let caps = vec![
        ...
        cap(
            "Dress this device in a skin: what is held, take one in, wear or drop one, check a file before handing it on",
            &["skin list", "skin add", "skin use", "skin rm", "skin validate", "skin template"],
        ),
        cap(
...
fn all_commands() -> Value {
    json!([
        ...
        json!({ "name": "skin list", "summary": "The skins this device holds, and which of them is on. A skin is a YAML file of token values kept at `<base>/skins/<name>.yaml`; the one that is on is a name in config.json (`config set skin`), never synced, because it points at a file on this machine. A file in the directory that will not read is listed as unreadable rather than left out.",
            "args": [], "flags": [{ "name": "--json", "help": "machine-readable output" }],
            "examples": ["amenbo skin list --json"] }),
        json!({ "name": "skin add", "summary": "Takes a skin file in, to `<base>/skins/<name>.yaml` under the name the file gives itself. It is read, checked and measured first. A key this build does not know, a name it keeps to itself and a value that is not text are dropped with a warning naming each; a later `skin_v`, a `themes` line that disagrees with the tables, a name that cannot be a filename, and an embedded font with no `license_text` turn the whole file away. A skin may carry one font (`font_file`: family/format/license/license_text/data as base64); it is taken only as woff2 under 2MB, and anything else about it drops the font with a warning and keeps the colours. The four ladders (text, spacing, radius, shadow) are not written out name by name — a skin sets `fs-scale` / `s-scale` / `r-scale` / `shadow-scale` and amenbo multiplies its own values, the first three held to 0.85-1.30 (measured). The frame pair is held to what this build draws: `border-style` takes solid/double/none, `border-w` is put back inside 0-4px, and a `double` frame is drawn at 3px whatever was asked for (the widths that split are not one range). A colour pairing under WCAG AA is reported with its number and does NOT stop the import — an author has to be able to try work in progress. A name already held is refused with both versions named, unless --yes replaces it.",
            "args": [{ "name": "path", "required": true, "help": "the .yaml to take in" }],
            "flags": [{ "name": "--yes/-y", "help": "replace the skin already held under that name" }, { "name": "--json", "help": "machine-readable output" }],
            "examples": ["amenbo skin add ./washi.amenbo-skin.yaml", "amenbo skin add ./washi.yaml --yes"] }),
        json!({ "name": "skin use", "summary": "Puts one of the held skins on, or takes whatever is on off (`none`). One is worn at a time, so this is a choice rather than a switch. A name nothing is held under is refused rather than recorded.",
            "args": [{ "name": "name", "required": true, "help": "the skin's name, or none" }],
            "flags": [{ "name": "--json", "help": "machine-readable output" }],
            "examples": ["amenbo skin use washi", "amenbo skin use none"] }),
        json!({ "name": "skin rm", "summary": "Takes a skin off the device. If it was the one on, it is taken off too — the config naming a file that is gone would say the device is wearing something it has not got. Removing a name nothing is held under is not a failure; it says so and stops.",
            "args": [{ "name": "name", "required": true, "help": "the skin's name" }],
            "flags": [{ "name": "--json", "help": "machine-readable output" }],
            "examples": ["amenbo skin rm washi"] }),
        json!({ "name": "skin validate", "summary": "Runs the reading, the check and the contrast measure over a file without taking it in — the author's face of what `skin add` does, so the reason a file will be turned away is learned before it is handed to anybody. Exits non-zero on a refusal; warnings and short pairings are reported and exit 0.",
            "args": [{ "name": "path", "required": true, "help": "the .yaml to read" }],
            "flags": [{ "name": "--json", "help": "machine-readable output" }],
            "examples": ["amenbo skin validate ./washi.yaml --json"] }),
        json!({ "name": "skin template", "summary": "Writes this build's own colours to stdout as a skin document — both sides, every colour a skin may set, already valid. A starting point that is the screen the author is looking at. The names that carry no colour (the type scale, spacing, font stacks) are theirs to add; `skin validate` says whether a name is one this build has.",
            "args": [], "flags": [{ "name": "--json", "help": "machine-readable output (the document under `yaml`)" }],
            "examples": ["amenbo skin template > my-skin.yaml"] }),
