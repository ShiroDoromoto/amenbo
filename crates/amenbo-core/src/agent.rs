...
fn all_commands() -> Value {
    json!([
        ...
        json!({ "name": "skin add", "summary": "Takes a skin file in, to `<base>/skins/<name>.yaml` under the name the file gives itself. It is read, checked and measured first. A key this build does not know, a name it keeps to itself and a value that is not text are dropped with a warning naming each; a later `skin_v`, a `themes` line that disagrees with the tables, and a name that cannot be a filename turn the whole file away. A colour pairing under WCAG AA is reported with its number and does NOT stop the import — an author has to be able to try work in progress. A name already held is refused with both versions named, unless --yes replaces it.",
