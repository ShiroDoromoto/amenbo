...
impl Instructor {
    ...
    fn action(&self, domain: Domain, op: &str, with: &Args) -> Result<String, String> {
        Ok(match (domain, op) {
            ...
            // reservation gets, and naming one of the three otherwise. The three are the pane's own,
            // drawn in the pane's own box — one loss at two sizes — so each is said here by what it
            // does rather than by what it reads, the way `remove-pane`'s are.
            //
