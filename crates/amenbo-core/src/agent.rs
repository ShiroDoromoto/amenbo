...
fn all_commands() -> Value {
    json!([
        ...
        cmd("automation step-out", "Puts down one thing this step produced, under the name its port was declared with. Written <name>=<value>, or <name> --file <path> for a file, which is attached to this step execution first. WHAT THE NAME WAS DECLARED TO CARRY DECIDES HOW THE WORDS ARE READ: on a value port they are the answer, and on a task_make port they name a task this step raised along the way, which is written as the task. The task the run is about does not come this way and is refused here, naming `automation step-take` — reserving it and declaring it are one act. A name the step did not declare is refused, and so is a payload of the wrong kind. Putting the same name down twice replaces the first. Which way out it belongs to is stamped at `done`, not here.",
            json!([{ "name": "<name>=<value>", "help": "what is handed on, or just <name> beside --file", "required": true },
                   ...
                   { "name": "--json", "help": "machine-readable output" }]),
            json!(["amenbo automation step-out note=\"nothing to fix\" --actor ai",
                   "amenbo automation step-out report --file ./report.md --actor ai",
                   "amenbo automation step-out \"a task to redo it\"=AMB-T-9001 --actor ai"])),
        cmd("automation step-done", "Says this step is finished: which way out it took, and what it did. The run reads the way out to decide what happens next — another step, the run closing, or the run stopping for a person. --exit left out is the unnamed way out, and a name the step did not declare is taken as the error one, the name kept in the record. A missing required output is refused, named, before anything is written. --out repeats for anything not already put down with `automation step-out`. Prints what the run does next.",
            json!([{ "name": "--report <text>", "help": "what this step did (`-` reads stdin)", "required": true },
                   ...
                   { "name": "--json", "help": "machine-readable output" }]),
            json!(["amenbo automation step-done --report - --exit \"something to fix\" --actor ai"])),
