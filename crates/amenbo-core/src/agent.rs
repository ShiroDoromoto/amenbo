...
fn capabilities() -> Value {
    let caps = vec![
        ...
        cap(
            ...
            &["automation list", "automation show", "automation action-list", "automation action-show", "automation builtin-list"],
        ),
        cap(
    ...
    json!([
        ...
        cmd("automation step-add", "Adds a step to a library action — one step is one terminal, and it carries its own prompt. Who carries it out is not the step's: it is chosen where the action is placed (`automation agent-set`), so the same action can be run by different agents on two automations. It is born with the done way out and the error one (`*`), and the first step of an action is the one the action opens first. --work-dir names the setting or the input the working folder is taken from: a name, not a path, and the name is one the action declares. A built-in is not put in here: it stands on a picture only as Amenbo's own action (`automation place-add --builtin`). Prints the id the action's own edge and wire commands take.",
            json!([{ "name": "<action>", "help": "action id", "required": true },
                   ...
                   { "name": "--no-task-notes", "help": "do not hand it the notes of the task the run is on" },
                   { "name": "--no-task-decisions", "help": "do not hand it the decisions linked to the task the run is on" },
                   { "name": "--no-task-comments", "help": "do not hand it the comments on the task the run is on" }]),
        ...
        cmd("automation step-update", "Changes a step. Only the fields given change. Rewriting the prompt reaches every placement of the action holding it, which is what the library is for — and is refused while a run placing that action is running or paused.",
            json!([{ "name": "<id>", "help": "step id", "required": true },
                   ...
                   { "name": "--task-notes <true|false>", "help": "whether it is handed the notes of the task the run is on" },
        ...
        cmd("automation exit-rename", "Renames a way out. Edges and wires key a way out by its id, so everything on it stays on it. A way out keeps a name — there is no leaving it without one. The error way out's name is fixed and refused at both ends.",
            json!([{ "name": "<id>", "help": "way out id", "required": true },
                   { "name": "--name <str>", "help": "the new name", "required": true }]),
        ...
        cmd("automation edge-add", "Says what happens after one box leaves through one way out: go on to another box (--to), leave the action by one of the ways out it declares (--exit-to, inside an action only — bare for its done way out), close the run (--done), or stop the run and call a person (--halt). Both pictures are drawn with this one verb: --in-action draws the line inside a library action, between its steps, and left out it is drawn on an automation, between its placements. The edge carries no condition of its own — the way out is the condition — and one way out says one thing, so a second edge on the same one is refused. --from is written as one token, <box>:<way out>: `4:` is the done way out and `4:*` the error one. --max-times caps how often a --to edge may be taken FOR ONE TASK, counted afresh at the next task; left out it is 10, which guards a loop that never converges without stopping a review that goes round three times. --no-max takes the cap off. A --exit-to, --done or --halt edge is taken once and carries no cap at all.",
            json!([{ "name": "--from <box>:<way out>", "help": "where it leaves from (`4:` is the done way out, `4:*` the error one)", "required": true },
                   { "name": "--in-action", "help": "draw it inside a library action, between its steps" },
                   { "name": "--to <id>", "help": "go on to this box" },
                   { "name": "--exit-to [name]", "help": "leave the action by the way out it declares under this name — bare, the done one (--in-action only)" },
        ...
        cmd("automation wire-add", "Joins what one way out hands on to what a later box takes in. Both ends are named rather than keyed: one action placed twice on an automation gives two placements whose ports carry the same names, so only the box, the way out and the port name together say which is meant. --from is the same <box>:<way out> token the edge takes, and --in-action picks the same picture it picks there. The two ends are refused unless they carry the same kind of thing. Inside an action (--in-action), either end may be `0`, the action itself: --from 0 hands an input the action declares to a step, and --to 0 fills an output on the way out of the action that --from's way out returns to — so that step's --exit-to edge is drawn first.",
            json!([{ "name": "--from <box>:<way out>", "help": "where it comes from (`4:` is the done way out; `0` is the action itself, with --in-action)", "required": true },
                   { "name": "--in-action", "help": "draw it inside a library action, between its steps" },
