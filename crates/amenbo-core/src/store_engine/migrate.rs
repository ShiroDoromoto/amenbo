...
/// v54: an action's declarations are **joined** to the step inside it (`AMB-T-5311`).
///
...
fn join_the_action_to_its_step(ctx: &Ctx<'_>) -> Result<()> {
    ...
    let lone_steps: Vec<(i64, i64)> = {
        let mut stmt = tx.prepare(
            "SELECT action_id, MIN(id) FROM automation_step GROUP BY action_id HAVING COUNT(*) = 1",
        )?;
        let rows = stmt.query_map([], |r| Ok((r.get(0)?, r.get(1)?)))?;
        rows.collect::<rusqlite::Result<Vec<_>>>()?
    };
...
mod tests {
    ...
    /// **v54 joins what an action declares to the step inside it** (`AMB-T-5311`).
    ///
