 ...
 * machine and is answered by giving it more watches. They belong to the folder rather than to the
 * list of what changed — the tree below is just as unwatched — so they are drawn beside the
 * heading, above both rows.
 */
function FolderSection({ projectId, root, label, bound, landing, onRead, onMenu }: {
  ...
  const [changes, setChanges] = useState<FolderChangesDto>(
    { root, changed: [], capped: false, unwatched: false, gone: false },
  );
  ...
  useEffect(() => {
    ...
    void folderWatch(projectId, root)
      ...
      .catch(() => {
        if (alive) setChanges({ root, changed: [], capped: false, unwatched: false, gone: false });
      });
    return () => {
      alive = false;
