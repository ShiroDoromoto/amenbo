...
- **Automations** — a picture drawn once and walked by agents: each step is a prompt and
  ...
  Taking a task, filing one, closing it, fetching what a run starts from, splitting by a
  classification and cutting a worktree are Amenbo's own built-ins, so a prompt is left with what
  needs an agent's judgement. A run starts from a task it takes, from what a person hands it, or
  from what it goes and fetches.
...
# A run comes in by one of three entrances, and each ends up working a task. The first thing placed
# on a picture is where a run starts, so it is one of three built-ins: take_task takes one its filter
# finds; make_task files one from what a person hands `start` (`--title/--notes/--dim`); fetch goes to
...
amenbo automation exit-add --action 7 --name "gave up" # a way out a placement may leave by (every one has the done one and `*`)
...
# The look, as a file. A skin is one zip — `skin.yaml` at its root beside the pictures, icons
...
amenbo skin write-out my-skin ./copy.zip    # the file it arrived in, byte for byte
...
The directory tree is in [CONTRIBUTING.md](CONTRIBUTING.md#the-repository-tree),
with the rest of what a contributor needs.
...
Pinned Node + Rust, one-shot setup, and the macOS-only GUI note:
[CONTRIBUTING.md](CONTRIBUTING.md#toolchain).

## Build and run

Build, the fast/full test split, the nextest + shell gates, and where data lives:
[CONTRIBUTING.md](CONTRIBUTING.md#build-and-run).
