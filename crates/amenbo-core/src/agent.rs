...
const JA_PHRASEBOOK: &[(&str, &str)] = &[
    ...
    ("Attach files (content-addressed blobs) or external links to tasks, decisions, or individual comments", "タスク・決定記録・個々のコメントにファイル（内容アドレスの blob）や外部リンクを添付する"),
    ...
    ("Open the latest installer to update (amenbo does not self-update)", "最新インストーラを開いて更新する（amenbo は自己更新しません）"),
    ...
    ("Physically erase content from the store — a comment in full, or one accepted decision's frozen body (human-gated maintenance)", "ストアから内容を物理的に消去する ── コメント全体、または採択済み決定 1 件の凍結本文（人間ゲートの保守作業）"),
    // command.summary
    ...
    ("Opens the latest amenbo installer for this OS to update (amenbo does not self-update). Resolves the one-piece installer URL (GUI + CLI) from the published latest.json — falling back to the releases page — and opens it in your browser.", "更新のために、この OS 向けの最新 amenbo インストーラを開きます（自己更新はしません）。公開 latest.json から現 OS の一体型インストーラ URL（GUI+CLI 同梱）を解決し（無ければ最新リリースページへフォールバック）、既定ブラウザで開きます。"),
    ...
    ("Run the lint on every commit, by installing it as a git hook (consent is asked once per project and remembered; amenbo touches only the hook it wrote)", "lint を git フックとして入れ、コミットのたびに走らせます（同意はプロジェクト単位で一度だけ聞いて憶えます・amenbo が触るのは自分が書いたフックだけ）"),
    ("Writes the git hooks that run `amenbo lint` on every commit: `pre-commit` reads the staged diff, and `commit-msg` reads the message, which is the only place git offers it (at pre-commit time no message exists yet). One lint, two of git's doors. It records this project's consent so the question is not asked again — consent is to the lint as a feature, not to a file, so one answer covers every slot. Installing means writing into your git plumbing, which amenbo does not do unasked: it offers this once per project on an interactive terminal, and this is the explicit face of the same thing — usable any time, including after a `no`. amenbo marks the hooks it writes and touches nothing else: a hook from husky, lefthook or your own hand is NEVER overwritten — install steps around it, wiring the slots it may own and naming the one line to add to the rest (`amenbo lint || exit 1`, or `amenbo lint \"$1\" || exit 1` for commit-msg). Only an install with no slot to write at all is refused. Re-running over amenbo's own hooks rewrites them, which is how a newer build's hooks land. They honour core.hooksPath, exit 0 when amenbo is not on PATH (a convenience, not a gate), and one commit is bypassed with `git commit --no-verify`.", "コミットのたびに `amenbo lint` を走らせる git のフックを書きます: `pre-commit` は staged diff を、`commit-msg` はコミット文を読みます（pre-commit の時点ではコミット文がまだ存在しないため、git がそれを渡すのは commit-msg だけです）。lint は1つ、git の入口が2つです。このプロジェクトの同意を記録して以後は聞きません——同意はファイルではなく **lint という機能**に対するものなので、1つの答えが全ての枠を覆います。インストールはあなたの git 配管への書き込みであり、amenbo は黙ってそれをしません: 対話端末でプロジェクトごとに一度だけ尋ねます。これはその明示の面で、いつでも——`no` と答えた後でも——使えます。amenbo は自分が書いたフックに印を付け、それ以外には触れません: husky・lefthook・自作のフックは**絶対に上書きしません**——install はそれを避けて通り、書いてよい枠だけを配線し、残りには足すべき1行（`amenbo lint || exit 1`、commit-msg なら `amenbo lint \"$1\" || exit 1`）を示します。書ける枠が1つも無い install だけが拒否されます。amenbo 自身のフックへの再実行は書き直しで、新しいビルドのフックはこうして入ります。フックは core.hooksPath に従い、amenbo が PATH に無ければ 0 で抜け（gate ではなく利便性のため）、1 回のコミットは `git commit --no-verify` で迂回できます。"),
    ("Removes the lint hooks amenbo wrote, and records that this project does not want them (so it is not asked again). The mirror of install, refusal for refusal and partial for partial: a hook amenbo did not write is not amenbo's to delete and is left alone, and only a call with nothing of ours to remove and a stranger in the way is refused. With no hooks of ours there, it records the answer and does nothing else. A `no` closes the question, not the door — `hooks install` works whenever you want them back.", "amenbo が書いた lint フックを削除し、このプロジェクトはそれを望まないと記録します（以後は聞きません）。install の鏡像で、拒否も部分成立も対称です: amenbo が書いていないフックは amenbo が消してよいものではなく、そのまま残します。自分のフックが1つも無く他人のフックだけがある呼び出しだけが拒否されます。自分のフックが無ければ、答えを記録するだけで他には何もしません。`no` は質問を閉じるだけで、扉は閉じません——`hooks install` はいつでも使えます。"),
    ("Shows the two facts side by side: what is in each hook slot (no hook / amenbo's, with its marker version / one amenbo did not write), and what this project answered (not asked yet / yes / no). They are independent on purpose — the record says what was answered and is NEVER read as a mirror of the disk, which is what makes a hook deleted or added by hand a state amenbo can see rather than one that breaks it. Read-only.", "2つの事実を並べて示します: フックの枠に何があるか（フック無し / amenbo のもの・印の版付き / amenbo が書いていないもの）と、このプロジェクトが何と答えたか（未回答 / yes / no）。この2つが独立なのは設計です——記録は「何と答えたか」を語るだけで、ディスクの鏡としては**決して**読みません。だから手で消された・足されたフックは、amenbo が破綻する事態ではなく、見て取れる状態になります。読み取り専用。"),
    ...
    ("Explicitly changes the progress state (todo/in_progress/done/blocked). Setting in_progress reserves the task: a compare-and-swap that succeeds only from todo, so a second session's reserve is rejected with already_reserved (the double-work guard), and only a ready task can be reserved, so an open blocker or an unsettled premise is rejected with not_ready (there is no --force). todo hands it back. done marks completed. blocked declares an external stall only — an unmet premise is derived as ready:no, never declared here.", "ステータスを明示的に変えます（todo/in_progress/done/blocked）。in_progress は着手の予約です: 現在 todo のときだけ成功する compare-and-swap で、別セッションの予約は already_reserved で弾かれ（二重着手ガード）、さらに予約できるのは ready なタスクだけなので、未完了のブロッカーや未確定の根拠があれば not_ready で弾かれます（--force は無い）。todo で手放します。done は完了にします。blocked は外部要因で止まっていることの宣言に限ります ── 前提の未達は ready:no として導出されるもので、ここで宣言しません。"),
    ...
    ("Rewrites a comment's body in place — the id, its place on the timeline, and its attachments all stay. What is frozen once a decision is accepted is the decision's own body, not the comments under it. Identify the comment by id; `decision comment list` prints it.", "コメント本文をその場で書き換えます── id・タイムライン上の位置・付いていた添付はそのまま残ります。採択で凍結されるのは決定自身の本文であって、その下のコメントではありません。コメントは id で指定します。id は `decision comment list` が表示します。"),
    ("the new body, as Markdown — it replaces the old one outright", "新しい本文（Markdown）── 元の本文をまるごと置き換えます"),
    ...
    ("Adds a comment to a decision's timeline. The decision's own body (conclusion + rationale) is frozen once accepted; comments are the way to discuss it afterwards or record accept/reject reasoning (`decision accept/reject --reason` is thin sugar over this).", "決定記録のタイムラインにコメントを追加します。決定自身の本文（結論＋根拠）は採択されると凍結されるので、その後の議論や採択・却下の理由の記録はコメントで行います（`decision accept/reject --reason` はこれの薄い糖衣です）。"),
    ...
    ("Attaches a file (content-addressed `blob`) or external link (--url) to a single TASK comment, kept separate from the parent task's own attachments so a comment's own attachment timeline is preserved. Same two modes as `task attach`. Identify the comment by id; find ids with `comment list <task> --json`. A decision comment is attached to with `decision comment attach` — the two comment tables number apart, so the command, not the id, says which table an id belongs to. List them with `attach ls --task-comment <id>`.", "単一の**タスク**コメントにファイル（内容アドレスの `blob`）や外部リンク（--url）を添付します。親タスク自身の添付とは分けて保持し、コメント固有の添付タイムラインを保ちます。`task attach` と同じ 2 モードです。コメントは id で指定します。id は `comment list <task> --json` で見つけます。決定記録のコメントには `decision comment attach` で添付します ── 2 つのコメント表は別々に採番するので、その id がどちらの表のものかは id ではなくコマンドが語ります。一覧は `attach ls --task-comment <id>` です。"),
    ("Attaches a file (content-addressed `blob`) or external link (--url) to a single DECISION comment — the mirror of `comment attach` (which takes task comments). Identify the comment by id; find ids with `decision comment list <decision> --json`. List them with `attach ls --decision-comment <id>`.", "単一の**決定記録**コメントにファイル（内容アドレスの `blob`）や外部リンク（--url）を添付します ── `comment attach`（タスクのコメントを取る）の鏡です。コメントは id で指定します。id は `decision comment list <decision> --json` で見つけます。一覧は `attach ls --decision-comment <id>` です。"),
    ...
    ("Edits a proposed decision's title/body. Once accepted a decision is frozen (append-only): `decision reopen` it back to proposed for a minor in-place fix, or supersede it when a new decision replaces it.", "議論中の決定のタイトル／本文を編集します。採択されると決定は凍結されます（追記のみ）: 軽微なその場修正なら `decision reopen` で議論中に戻し、新しい決定が置き換えるなら supersede します。"),
    ...
    ("Reopens an accepted decision for editing (accepted → proposed; clears decided_at/decided_by). The canonical light path when a just-accepted decision needs a minor fix (typo, stale status line): reopen, `decision edit`, then re-accept — instead of a supersede that clutters the chain. Non-destructive; the frozen invariant stays (you must reopen explicitly first). No-op if already proposed; refused for rejected decisions. A decision another one supersedes is not frozen by that fact (currency is derived, not a status), so it can be reopened too.", "採択済みの決定を編集のため再オープンします（accepted → proposed・decided_at/decided_by をクリア）。採択直後の決定に軽微な修正（誤字、古い状態行）が要るときの正典の軽量経路です: チェーンを散らかす supersede でなく、reopen → `decision edit` → 再採択。非破壊です。凍結の不変条件は保たれます（先に明示的に reopen が必要）。すでに proposed なら何もしません。rejected の決定には拒否します。覆された決定は status では凍らない（現行性は派生）ので reopen できます。"),
    ...
    ("Attaches a file or external link to a task. A file is ingested as a content-addressed `blob` (the bytes are copied into the store keyed by their BLAKE3 digest; the truth source records only metadata — hash/filename/mime/size); --url instead records an external link (`url` mode, not managed). MIME is guessed from the file extension. The blob is checked against the per-file size cap before ingest. Manage attachments with `attach ls/show/open/rm`.", "タスクにファイルや外部リンクを添付します。ファイルは内容アドレスの `blob` として取り込まれます（バイト列は BLAKE3 ダイジェストをキーにストアへ複製され、真実源はメタデータ ── ハッシュ/ファイル名/mime/サイズ ── だけを記録）。--url は代わりに外部リンクを記録します（`url` モード・管理下ではない）。MIME はファイル拡張子から推測します。blob は取り込み前にファイルごとのサイズ上限で検査されます。添付は `attach ls/show/open/rm` で管理します。"),
    ("Attaches a file (content-addressed `blob`) or external link (--url) to a decision record. Same two modes as `task attach`. Manage with `attach ls/show/open/rm`.", "決定記録にファイル（内容アドレスの `blob`）や外部リンク（--url）を添付します。`task attach` と同じ 2 モードです。`attach ls/show/open/rm` で管理します。"),
    ("Lists the attachments on a task, decision, or a single comment, in attach order. A comment is named by a flag, not by the positional target: the task and decision comment tables number apart, so a bare id cannot say which table it belongs to.", "タスク・決定記録・単一コメントの添付を、添付順で一覧します。コメントは位置引数ではなくフラグで指します: タスクと決定記録のコメント表は別々に採番するので、裸の id ではどちらの表のものか言えません。"),
    ...
    ("Opens an attachment — a blob via the OS default opener, or the external URL. A blob whose bytes are not present locally reports not_found.", "添付を開きます ── blob は OS の既定オープナで、外部 URL はそのまま。バイト列がローカルに無い blob は not_found を返します。"),
    ...
    ("Redacts an accepted decision's frozen body: overwrites it with the given text in place (the prior body is physically replaced, not merely superseded), then VACUUMs — so one section can be removed while the decision keeps its number, links and other fields. The replacement body comes from --body, --body-file, or stdin. Destructive maintenance: takes a safety backup first (a `pre-erase-*.amenbo-backup` archive next to the store, which `restore` puts the store back from — only the newest is kept), confirms unless --yes, and is refused for the AI actor (a human must run it). The safety backup still holds the old body — delete it after verifying.", "採択済み決定の凍結本文を墨消しします: 与えたテキストでその場を上書きし（以前の本文は supersede でなく物理的に置換）、その後 VACUUM します ── これで決定が番号・結び・他フィールドを保ったまま、1 節だけ除けます。置換本文は --body、--body-file、または標準入力から取ります。破壊的な保守作業: まず安全スナップショットを取り、--yes が無ければ確認し、AI アクターには拒否されます（人間が実行する必要）。安全スナップショットにはまだ古い本文が残るので、確認後に削除してください。"),
    ...
    ("new name", "新しい名前"),
    ("new notes (Markdown)", "新しいメモ（Markdown）"),
    ("value name", "値名"),
    ...
    ("reason it is stuck (recorded as a comment)", "行き詰まった理由（コメントとして記録）"),
    ...
    ("comment body (Markdown)", "コメント本文（Markdown）"),
    ...
    ("conclusion + rationale (Markdown)", "結論＋根拠（Markdown）"),
    ...
    ("reason for accepting (recorded as a decision comment)", "採択の理由（決定コメントとして記録します）"),
    ("reason for rejecting (recorded as a decision comment)", "却下の理由（決定コメントとして記録します）"),
    ("new title", "新しいタイトル"),
    ("new body", "新しい本文"),
...
pub fn build() -> Value {
    json!({
        ...
        "operating": [
            ...
            "Task-ify the request: when the user asks for something substantive, record it as a task (task add), keep its status current as you work (in_progress → done), and leave the outcome on its timeline (comment add) so the next session can pick up. The backlog — not the chat log or your own memory — is where the work and its state live.",
        ...
        "agentCycle": [
            ...
            "1. list: your mailbox is `amenbo task list --filter \"assignee:me-ai status:todo ready:yes\" --sort priority --json`. Take the highest-priority task. (`status:todo` is fresh, unreserved work — a task already `in_progress` is one another session is on, so it stays out of the mailbox and you never double-book; `blocked` is deliberately out, being an external stall — a second machine, a human go/no-go — that you should not self-assign. Waiting on a ruling is not one of those: an unsettled premise is derived as `ready:no`, never declared as `blocked`. `ready:yes` hides work whose declared premises are unmet — an open blocker, or a linked decision that is not settled — so you never grab it early; query `ready:no` to see each task's blocked_by_open and blocked_by_decisions.)",
            "2. reserve: `amenbo task status <id> in_progress` (todo→in_progress), then re-confirm it is in_progress with `amenbo task show <id>` before starting. `status` is the whole double-work guard, and reserving is a compare-and-swap: `→ in_progress` succeeds only when the task is currently `todo`. If another session reserved it first, your reserve is rejected with `already_reserved` (a non-zero exit) instead of silently succeeding — so the collision is actually detected. The reserve also requires `ready`, so a task with an open blocker or an unsettled premise is rejected with `not_ready` (also a non-zero exit), and there is no `--force`. The two failures pull in opposite directions. On `already_reserved` the task is taken by someone else: go back to step 1 and pick the next one. On `not_ready` it is your own declaration that holds it: resolve the premise — finish the blocker or `task undepend` it; `decision accept` the linked decision, or `decision link --unlink` it — and reserve only then. Both guards judge this transition only: an edge added, or a decision rejected, under a running task never strips its status, and `→ todo` / `→ blocked` / `→ done` stay unconditional, so the hand-back path is never closed.",
    ...
    json!({
        ...
        "taskShaping": {
            ...
            "optional": [
                ...
                optional_step("you are handing work to a person or that person's AI", "Delegate at creation (--to/--ai) or afterward.", &["task add", "task assign"]),
        ...
        "decision": {
            ...
            "optional": [
                ...
                optional_step("a proposed decision is agreed and ready to freeze", "Accept it (stamps decided_at/decided_by; accepted decisions are then frozen).", &["decision accept"]),
                optional_step("a just-accepted decision needs a minor fix (typo / stale line)", "Reopen it to proposed, edit it, then re-accept — a light, audited path that avoids a supersede that would clutter the chain.", &["decision reopen", "decision edit", "decision accept"]),
        ...
        "executionExceptions": {
            ...
            "optional": [
                ...
                optional_step("your reserve was rejected with not_ready", "Resolve the premise you declared, rather than working around it: finish the blocker or drop the edge; get the linked decision accepted, unlink it, or relink it to its successor. There is no --force.", &["task done", "task undepend", "decision accept", "decision link"]),
        ...
        "commit": {
            ...
            "optional": [
                optional_step("the human wants every commit linted without anyone having to remember", "Offer the lint hooks (opt-in, one question per project).", &["hooks install", "hooks status"]),
            ]
        },
        "worktree": {
...
fn capabilities() -> Value {
    let caps = vec![
        ...
        cap(
            "Attach files (content-addressed blobs) or external links to tasks, decisions, or individual comments",
            &["task attach", "decision attach", "comment attach", "decision comment attach", "attach ls", "attach show", "attach open", "attach rm"],
        ),
        cap(
        ...
        cap("Open the latest installer to update (amenbo does not self-update)", &["update"]),
        ...
        cap(
            "Run the lint on every commit, by installing it as a git hook (consent is asked once per project and remembered; amenbo touches only the hook it wrote)",
        ...
        cap(
            "Physically erase content from the store — a comment in full, or one accepted decision's frozen body (human-gated maintenance)",
...
fn all_commands() -> Value {
    json!([
        ...
        cmd("update", "Opens the latest amenbo installer for this OS to update (amenbo does not self-update). Resolves the one-piece installer URL (GUI + CLI) from the published latest.json — falling back to the releases page — and opens it in your browser.",
            ...
            json!(["amenbo update", "amenbo update --print", "amenbo update --print --json"])),
        ...
        json!({ "name": "dimension update", "summary": "Updates a dimension's name, notes, value ordering, and/or time-axis role. Only the given fields change.",
            "args": [{ "name": "id", "required": true, "help": "dimension id or name" }],
            "flags": [{ "name": "--name <str>", "help": "new name" },
                      { "name": "--notes <str>", "help": "new notes (Markdown)" },
        ...
        json!({ "name": "task update", "summary": "Updates a task.",
            "args": [{ "name": "id", "required": true, "help": "task ID" }],
        ...
        json!({ "name": "task block", "summary": "Marks blocked (stuck) — for an external stall only (a second machine, a human go/no-go); an unmet premise is derived as ready:no instead. --reason is recorded as a comment.",
            "args": [{ "name": "id", "required": true, "help": "task ID" }],
            "flags": [{ "name": "reason", "help": "reason it is stuck (recorded as a comment)" }],
        ...
        json!({ "name": "comment edit", "summary": "Rewrites a comment's body in place — the id, its place on the timeline, and its attachments all stay, so links to it keep resolving. Prefer this over deleting and re-posting when you only need to fix what a comment says. Identify the comment by id; `comment list` prints it.",
            "args": [{ "name": "comment", "required": true, "help": "target comment ref, AMB-C-n (from `comment list`)" }],
            "flags": [{ "name": "--text <str>", "required": true, "help": "the new body, as Markdown — it replaces the old one outright" }],
        ...
        json!({ "name": "comment add", "summary": "Adds a comment to a task.",
            ...
            "flags": [{ "name": "--text <str>", "required": true, "help": "comment body (Markdown)" }],
        ...
        json!({ "name": "decision add", "summary": "Records a new decision (proposed) — an append-only \"why we chose X\". A decision is a Task sibling (project-scoped), NOT a task: it has no mailbox workflow and never appears in task lists. Decisions have their own device-global number space, shown as AMB-D-N (tasks are AMB-T-N); the kind code keeps AMB-D-<n> / AMB-T-<n> unambiguous. The body should be the conclusion + rationale (compress; do not paste raw discussion, and keep PII out). The project defaults to the bound project.",
            "args": [], "flags": [{ "name": "--title <str>", "required": true, "help": "decision title" },
                                   { "name": "--body <str>", "help": "conclusion + rationale (Markdown)" },
        ...
        json!({ "name": "decision edit", "summary": "Edits a proposed decision's title/body. Once accepted a decision is frozen (append-only): `decision reopen` it back to proposed for a minor in-place fix, or supersede it when a new decision replaces it.",
            "args": [{ "name": "id", "required": true, "help": "decision ref (AMB-D-n)" }],
            "flags": [{ "name": "--title <str>", "help": "new title" }, { "name": "--body <str>", "help": "new body" }],
        ...
        json!({ "name": "decision accept", "summary": "Accepts a decision (proposed → accepted); stamps decided_at/decided_by. --reason records the reason for accepting as a decision comment — the reason lives on the timeline, not in a dedicated field.",
            "args": [{ "name": "id", "required": true, "help": "decision ref (AMB-D-n)" }],
            "flags": [{ "name": "--reason <str>", "help": "reason for accepting (recorded as a decision comment)" }],
        ...
        json!({ "name": "decision reject", "summary": "Rejects a decision (proposed → rejected). --reason records the reason for rejecting as a decision comment — the reason lives on the timeline, not in a dedicated field.",
            "args": [{ "name": "id", "required": true, "help": "decision ref (AMB-D-n)" }],
            "flags": [{ "name": "--reason <str>", "help": "reason for rejecting (recorded as a decision comment)" }],
        ...
        json!({ "name": "decision reopen", "summary": "Reopens an accepted decision for editing (accepted → proposed; clears decided_at/decided_by). The canonical light path when a just-accepted decision needs a minor fix (typo, stale status line): reopen, `decision edit`, then re-accept — instead of a supersede that clutters the chain. Non-destructive; the frozen invariant stays (you must reopen explicitly first). No-op if already proposed; refused for rejected decisions. A decision another one supersedes is not frozen by that fact (currency is derived, not a status), so it can be reopened too.",
            "args": [{ "name": "id", "required": true, "help": "decision ref (AMB-D-n)" }],
        ...
        json!({ "name": "decision comment edit", "summary": "Rewrites a comment's body in place — the id, its place on the timeline, and its attachments all stay. What is frozen once a decision is accepted is the decision's own body, not the comments under it. Identify the comment by id; `decision comment list` prints it.",
            "args": [{ "name": "comment", "required": true, "help": "target comment ref, AMB-C-n (from `decision comment list`)" }],
            "flags": [{ "name": "--text <str>", "required": true, "help": "the new body, as Markdown — it replaces the old one outright" }],
            "examples": ["amenbo decision comment edit AMB-C-<n> --text \"Corrected: the benchmark was 10k, not 1k\""] }),
        json!({ "name": "decision comment add", "summary": "Adds a comment to a decision's timeline. The decision's own body (conclusion + rationale) is frozen once accepted; comments are the way to discuss it afterwards or record accept/reject reasoning (`decision accept/reject --reason` is thin sugar over this).",
        ...
        json!({ "name": "decision attach", "summary": "Attaches a file (content-addressed `blob`) or external link (--url) to a decision record. Same two modes as `task attach`. Manage with `attach ls/show/open/rm`.",
            "args": [{ "name": "id", "required": true, "help": "target decision ref (AMB-D-n)" },
                     { "name": "source", "required": true, "help": "file path to ingest, or the external URL with --url" }],
            "flags": [{ "name": "--url", "help": "treat <source> as an external URL link instead of ingesting a file" },
                      { "name": "--name <str>", "help": "display label (defaults to the file name / URL)" }],
            "examples": ["amenbo decision attach AMB-D-<n> ./benchmark.csv"] }),
        cmd("attach ls", "Lists the attachments on a task, decision, or a single comment, in attach order. A comment is named by a flag, not by the positional target: the task and decision comment tables number apart, so a bare id cannot say which table it belongs to.",
        ...
        cmd("attach open", "Opens an attachment — a blob via the OS default opener, or the external URL. A blob whose bytes are not present locally reports not_found.",
            json!([{ "name": "id", "required": true, "help": "attachment id" }]),
