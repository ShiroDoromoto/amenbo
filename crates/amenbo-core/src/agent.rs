...
const JA_PHRASEBOOK: &[(&str, &str)] = &[
    ...
    ("Changes a configuration value. Known keys: default_view, language, date_locale (how dates are written, as a BCP-47 tag; unset follows language — the GUI reads it, the CLI never does), human_name, ai_name (the display names of the two actors — this is the only way to rename either), human_avatar, ai_avatar (their icons, as a data:image/png;base64 URI), ai_allow_project_ops, startup_integrity_check (read-only integrity doctor at open; warnings only; default on), update_check (checks a static latest.json for a newer release; infra-side only — no user data; timeout + silent-fail + cached; default on; AMENBO_UPDATE_CHECK=0 overrides).", "設定値を変更します。既知のキー: default_view, language, date_locale（日付の書き方。BCP-47 のタグ。未設定なら language に従う── 読むのは GUI だけで CLI は読まない）, human_name, ai_name（2 つの主体の表示名 ── 改名する口はここだけです）, human_avatar, ai_avatar（そのアイコン。data:image/png;base64 の URI）, ai_allow_project_ops, startup_integrity_check（オープン時に走る読み取り専用の整合性ドクター。警告のみ・既定オン）, update_check（静的 latest.json を照会して新しいバージョンに気付く。インフラ面のみ・ユーザーデータ非搭載・タイムアウト/失敗サイレント/キャッシュ・既定オン・AMENBO_UPDATE_CHECK=0 で無効化）。"),
    ...
    ("Speaks MCP (JSON-RPC on stdin/stdout) so an AI whose host cannot open a folder can still reach one. A host starts it, never a hand: typing it on a terminal only leaves a process waiting for a protocol nobody is speaking. It is a mediator and not a second amenbo — every tool call re-runs this same executable in the folder --dir names and hands back what that run wrote, so the startup, the integrity check and the reach are the CLI's own, and a folder that is not bound is refused in the words a person typing there would read. One server serves one folder: a host that wants two projects writes two entries, which is what keeps picking a project a person's word rather than an AI's. Three tools: `agent` (how to work here, in full), `agent_command` (one command's spec), and `run`, which types the caller's own words at any command. Two things are named rather than passed through — the facet is the server's to declare, so a `--actor` the caller wrote is dropped and `ai` put in its place; and `bind` and `init` are refused, either of them being a way for an AI to re-point the folder it was given and step outside it. Nothing else is added: `--yes` in particular is not, so a destructive command still stops at the confirmation a person gives.", "MCP（標準入出力の上の JSON-RPC）を話します。フォルダを開けないホストの AI からでも、1 つのフォルダに届くようにするためのものです。起動するのはホストであって、手ではありません。端末で打っても、誰も話さないプロトコルを待つプロセスが残るだけです。これは仲介役であって、2 つめの amenbo ではありません。ツール呼び出しのたびに、この実行ファイル自身を --dir が指すフォルダで実行し直し、その実行が書いたものをそのまま返します。ですから起動処理も整合検査も見える範囲も CLI 自身のもので、束縛されていないフォルダは、そこで人が打ったときと同じ言葉で断られます。1 つのサーバが担当するフォルダは 1 つです。2 つのプロジェクトを使いたいホストは、項目を 2 つ書きます。プロジェクトを選ぶのを人間の語彙にとどめ、AI の語彙にしないのは、これです。ツールは 3 つ。`agent`（ここでの働き方の全体）、`agent_command`（コマンド 1 つぶんの説明）、そして呼び出し側の言葉をどのコマンドにも打ち込む `run` です。そのまま渡さず名指しで扱うものが 2 つあります。1 つは実行者の指定で、これはサーバが自分で付けるものなので、呼び出し側が書いた `--actor` は捨てて `ai` を入れ直します。もう 1 つは `bind` と `init` で、どちらも AI が渡されたフォルダを別のプロジェクトに向け替えて外へ出る道なので拒否します。それ以外は足しません。とくに `--yes` は付けないので、壊す側のコマンドは人間が与える確認の前で止まります。"),
    ("the folder this server works in — every tool call runs there, and its `.amenbo` decides the project", "このサーバが担当するフォルダ。ツール呼び出しはすべてここで実行され、プロジェクトはそこの `.amenbo` が決めます"),
    (
    ...
    ("Adds a dimension (a user-defined classification axis) to a project. New projects seed no dimensions — create the axes you need. An axis is single-select; --ordered gives the values an explicit order, --time-axis marks it as the ordered time lane, --show-on-card marks it to show on the task card.", "プロジェクトに次元（ユーザー定義の分類軸）を追加します。新規プロジェクトは次元を持たないので、必要な軸を作ってください。軸は単一選択です。--ordered は値に明示的な順序を与え、--time-axis は順序付き時間レーンとして印を付け、--show-on-card はタスクカードに出す軸として印を付けます。"),
    ("Lists a project's dimensions in display order, each with its values.", "プロジェクトの次元を表示順で、それぞれの値とともに一覧します。"),
    ("Shows a dimension: name, notes, kind (single-select, ordered, time-axis, show-on-card), and its values.", "次元を表示します: 名前、メモ、種類（単一選択、順序付き、時間軸、カードに出す）、そして値。"),
    ("Updates a dimension's name, notes, value ordering, time-axis role, and/or whether it goes on the task card. Only the given fields change.", "次元の名前・メモ・値の順序・時間軸の役割・タスクカードに出すかどうかを更新します。指定したフィールドだけが変わります。"),
...
fn all_commands() -> Value {
    json!([
        ...
        json!({ "name": "config set", "summary": "Changes a configuration value. Known keys: default_view, language, date_locale (how dates are written, as a BCP-47 tag; unset follows language — the GUI reads it, the CLI never does), human_name, ai_name (the display names of the two actors — this is the only way to rename either), human_avatar, ai_avatar (their icons, as a data:image/png;base64 URI), ai_allow_project_ops, startup_integrity_check (read-only integrity doctor at open; warnings only; default on), update_check (checks a static latest.json for a newer release; infra-side only — no user data; timeout + silent-fail + cached; default on; AMENBO_UPDATE_CHECK=0 overrides).",
        ...
        json!({ "name": "mcp", "summary": "Speaks MCP (JSON-RPC on stdin/stdout) so an AI whose host cannot open a folder can still reach one. A host starts it, never a hand: typing it on a terminal only leaves a process waiting for a protocol nobody is speaking. It is a mediator and not a second amenbo — every tool call re-runs this same executable in the folder --dir names and hands back what that run wrote, so the startup, the integrity check and the reach are the CLI's own, and a folder that is not bound is refused in the words a person typing there would read. One server serves one folder: a host that wants two projects writes two entries, which is what keeps picking a project a person's word rather than an AI's. Three tools: `agent` (how to work here, in full), `agent_command` (one command's spec), and `run`, which types the caller's own words at any command. Two things are named rather than passed through — the facet is the server's to declare, so a `--actor` the caller wrote is dropped and `ai` put in its place; and `bind` and `init` are refused, either of them being a way for an AI to re-point the folder it was given and step outside it. Nothing else is added: `--yes` in particular is not, so a destructive command still stops at the confirmation a person gives.",
            "args": [],
            "flags": [{ "name": "--dir <path>", "required": true, "help": "the folder this server works in — every tool call runs there, and its `.amenbo` decides the project" }],
            "examples": ["amenbo mcp --dir ~/work/site-redesign"] }),
        ...
        cmd("dimension add", "Adds a dimension (a user-defined classification axis) to a project. New projects seed no dimensions — create the axes you need. An axis is single-select; --ordered gives the values an explicit order, --time-axis marks it as the ordered time lane, --show-on-card marks it to show on the task card.",
        ...
        json!({ "name": "dimension show", "summary": "Shows a dimension: name, notes, kind (single-select, ordered, time-axis, show-on-card), and its values.",
            "args": [{ "name": "id", "required": true, "help": "dimension id or name" }],
            "flags": [{ "name": "--json", "help": "machine-readable output" }],
            "examples": ["amenbo dimension show Category --json"] }),
        json!({ "name": "dimension update", "summary": "Updates a dimension's name, notes, value ordering, time-axis role, and/or whether it goes on the task card. Only the given fields change.",
            "args": [{ "name": "id", "required": true, "help": "dimension id or name" }],
