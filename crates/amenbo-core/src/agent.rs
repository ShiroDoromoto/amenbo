...
const JA_PHRASEBOOK: &[(&str, &str)] = &[
    ...
    ("Shows this store's configuration: where its files are, and every setting `config set` writes — the same settings `--json` carries, each line naming the key alongside its value, so a value that was set can be read back by whoever set it.", "このストアの設定を表示します: ファイルの置き場所と、`config set` が書ける設定すべて —— `--json` が載せるのと同じ設定を、キーと値を並べた行で出すので、設定した値をその人が読み戻せます。"),
    ...
    ("how many of this axis's values one record may hold: `single` (the default) or `multi`. Refused alongside --time-axis", "1件のレコードがこの軸の値をいくつ持てるか: `single`（既定）か `multi`。--time-axis と併せると拒否されます"),
    ("how many of this axis's values one record may hold (`single` / `multi`). Widening is free; going back to single is refused while any record answers with several, and the refusal names how many — clear the extra values off them first. Refused on the time axis either way", "1件のレコードがこの軸の値をいくつ持てるか（`single` / `multi`）。広げるのは自由です。single へ戻すのは、複数の値を持つレコードが1件でもあれば拒否し、その件数を名乗ります ── 先に余分な値を外してください。時間軸ではどちらの向きも拒否されます"),
    ("Lists a project's dimensions in display order, each with its readable key (slug) and its open values. Closed values are left out until --closed asks for them.", "プロジェクトの次元を表示順で、読める鍵（slug）と開いている値とともに一覧します。閉じた値は --closed で求めるまで出ません。"),
    ("Shows a dimension: name, its readable key (slug), notes, kind (single or multi, ordered, time-axis, closable, show-on-card, required, and the one side it is narrowed to, if it is), and its open values with their own keys. Closed values are left out until --closed asks for them.", "次元を表示します: 名前、読める鍵（slug）、メモ、種類（single か multi、順序付き、時間軸、閉じられる、カードに出す、必須、そして片側に絞ってあればその側）、そして開いている値とその鍵。閉じた値は --closed で求めるまで出ません。"),
    ("which side of the store this axis classifies (default: both)", "この軸がストアのどちら側に効くか（既定: both）"),
    ("which side of the store this axis classifies; narrowing it leaves the assignments already made on the other side in place, meaning nothing", "この軸がストアのどちら側に効くか。絞っても、もう片側に付いていた割り当ては残ります ── 意味を持たなくなるだけです"),
    ("Updates a dimension's name, notes, how many of its values one record may hold, value ordering, its role (time axis or closable), whether it goes on the task card, whether it must be answered, which side of the store it classifies, and/or its readable key. Only the given fields change.", "次元の名前・メモ・1件のレコードが持てる値の数・値の順序・役割（時間軸か、閉じられる軸か）・タスクカードに出すかどうか・答えを必須にするかどうか・ストアのどちら側に効くか・読める鍵を更新します。指定したフィールドだけが変わります。"),
    ...
    ("mark this axis as one whose values can be closed — retired from what a record is newly filed under, keeping everything already filed under them. One axis holds one role, so this and --time-axis are refused together", "値を閉じられる軸として印を付けます ── 閉じた値には新しく分類できなくなりますが、すでにその値で分類されているものはそのまま残ります。1つの軸が持つ役割は1つなので、--time-axis と同時には指定できません"),
    ("name this axis one whose values can be closed, or unname it. The two role flags name one slot, so each reads only its own half — --time-axis false leaves a closable axis closable. An axis that gives the role up keeps whatever was closed under it closed; reopening is free on any axis", "値を閉じられる軸として指名する、または指名を外します。役割のフラグ2つは同じ1枠を指すので、それぞれ自分の側しか読みません ── --time-axis false は、閉じられる軸を閉じられるまま残します。役割を手放した軸でも、閉じてあった値は閉じたままです。開き直すのはどの軸でも自由です"),
    ("list the closed values too, each marked [closed]", "閉じた値も一覧します（それぞれ [closed] が付きます）"),
    ("show the closed values too, each marked [closed]", "閉じた値も表示します（それぞれ [closed] が付きます）"),
    ("Closes a dimension value: nothing is newly filed under it, and everything already filed under it keeps it — the name, the key, the place in the order and every assignment stay, so a filter naming it goes on resolving. That is what closing is for, and what deleting cannot do. Only an axis marked closable (dimension add/update --closable) can close a value, and a required axis keeps one open value to offer: closing the last of them would leave a demand nobody can meet, so lower the requirement first.", "次元の値を閉じます: 以後その値には新しく分類できなくなり、すでにその値で分類されているものはそのまま残ります ── 名前も鍵も並び順の位置も割り当ても残るので、その値を名指す絞り込みは効いたままです。それが閉じる目的であり、削除にはできないことです。閉じられるのは closable の印が付いた軸（dimension add/update --closable）の値だけです。必須の軸は差し出せる値を1つ残します: 開いている最後の1つを閉じると誰にも答えられない要求が残るので、先に必須を下ろしてください。"),
    ("Reopens a closed dimension value, so records can be filed under it again. Free on any axis, whatever role it carries now — an axis that gave the closable role up would otherwise strand what was closed under it. Name a closed value the way you name an open one: it is out of the default listing, not out of reach (dimension list --closed shows it).", "閉じた次元の値を開き直し、また分類に使えるようにします。いまどの役割を持つ軸でも自由に打てます ── そうでないと、closable を手放した軸の下で閉じた値が取り残されます。閉じた値の名指し方は開いている値と同じです: 既定の一覧に出ないだけで、届かないわけではありません（dimension list --closed で出ます）。"),
    ("Deletes a dimension value permanently; its task assignments go with it unless --reassign-to moves them to another value of the same axis (alias: value-delete). A required axis refuses to empty tasks out: with tasks classified as the value, --reassign-to is required, and its last open value is refused outright — a closed value is not an answer, so it does not count, and a requirement nobody can answer would stop every creation on the project, so lower it first.", "次元の値を完全に削除します。そのタスクへの割り当ても一緒に消えますが、--reassign-to で同じ軸の別の値へ移せます（別名: value-delete）。必須の軸はタスクを値なしにさせません。その値が付いたタスクが1件でもあれば --reassign-to が要ります。開いている最後の1つはそもそも削除できません。閉じた値は答えにならないので数に入りません。誰にも答えられない必須はそのプロジェクトの作成をすべて止めてしまうので、先に必須を下ろしてください。"),
    ("Assigns a task or a decision a value of a dimension — one axis and one set of values, whichever of the two is being filed. What happens to what was there is the axis's own answer: a single axis replaces the value it held, a multi one keeps it and adds this. Taking one off is dimension unset either way. The target must say which kind it is: a bare number is refused, tasks and decisions numbering independently.", "タスクにも決定記録にも次元の値を割り当てます。軸も値も両者で同じものを使います。前に付いていた値がどうなるかは軸自身が決めます: single の軸は持っていた値を置き換え、multi の軸はそれを残してこの値を足します。外すのはどちらも dimension unset です。対象はどちらの種類かを名乗る必要があります: タスクと決定記録は別々に採番されるため、裸の数字は拒否されます。"),
    ("Clears a task's or a decision's value of a dimension. The target names its kind, as it does on set.", "タスクまたは決定記録の次元の値をクリアします。対象が種類を名乗る点は set と同じです。"),
    ("task or decision ref (AMB-T-n / AMB-D-n); a bare number is refused", "タスクまたは決定記録の参照（AMB-T-n / AMB-D-n）。裸の数字は拒否されます"),
    ...
    ("Records a new decision (proposed) — an append-only \"why we chose X\". A decision is a Task sibling (project-scoped), NOT a task: it has no mailbox workflow and never appears in task lists. Decisions have their own device-global number space, shown as AMB-D-N (tasks are AMB-T-N); the kind code keeps AMB-D-<n> / AMB-T-<n> unambiguous. The body should be the conclusion + rationale (compress; do not paste raw discussion, and keep PII out). The project defaults to the bound project. Classify it here with --dim: an axis the project marked required is read at `decision accept`, so one left empty is what turns the acceptance away — filling it at creation is what saves that round trip. The response names any required axis still blank (`unmet_required_dimensions` in --json), since a decision has no second stage to be turned away at and the one who accepts it is not the one who wrote it.", "新しい決定を記録します（議論中）── 追記のみの「なぜ X を選んだか」。決定はタスクの兄弟（プロジェクト単位）であってタスクではありません: 受信箱のワークフローを持たず、タスク一覧には現れません。決定は端末で一意の番号空間を持ち AMB-D-N で表示されます（タスクは AMB-T-N）。種別コードで AMB-D-<n> / AMB-T-<n> が曖昧になりません。本文は結論＋根拠にします（圧縮する・議論の生ログを貼らない・PII を入れない）。プロジェクトは紐付いたプロジェクトを既定にします。分類は --dim でここで付けます: プロジェクトが必須にした軸は decision accept が読むので、空のままだと採択が断られます。作成時に埋めておけばその往復が要りません。まだ空の必須軸は応答が名指しします（--json では `unmet_required_dimensions`）── 決定には断られる二段目が無く、採択を押すのは書いた本人ではないからです。"),
    ("Lists decisions (status:proposed|accepted|rejected, superseded:yes|no — the edge itself, so superseded:yes lists the decisions another decision draws a supersedes edge at — project:, dim:<axis>=<value> and its time_axis:<value> sugar, which read the same axes the tasks are classified on and fold the same way (different axes AND, the same axis ORs), decided_before:/decided_after: over the day a decision was accepted (YYYY-MM-DD, or today/-30d; both ends inclusive; a decision that was never accepted has no such day and matches neither)). Words are not among the keys — search <word> --kind decision is what finds where they are written. To ask which policies were settled by a date, compose this filter with superseded: — there is no separate as-of switch, and the composition recovers neither status transitions nor deleted decisions. Sort by decided/created/number/title/status (prefix - for descending; default -created). --limit/--offset page (JSON carries total_matched = the count before paging, count = this page). --with-body adds each decision's body to the rows — a projection that composes with --filter/--limit/--offset (narrow by status and page; it does not dump the whole corpus).", "決定記録を一覧します（status:proposed|accepted|rejected、superseded:yes|no ── エッジそのもので引きます。superseded:yes ＝ 別の決定が supersedes エッジを張っている決定、project:、dim:<軸>=<値> とその糖衣の time_axis:<値> ── タスクの分類と同じ軸を、同じ重ね方で読みます〔別の軸は AND・同じ軸は OR〕、decided_before:/decided_after: は採択された日で絞ります〔YYYY-MM-DD または today/-30d・両端を含む。採択されていない決定はその日を持たず、どちらにも一致しません〕）。語はキーにありません ── どこに書かれているかは search <語> --kind decision が答えます。「T までに決まっていた方針」は、このフィルタと superseded: の合成で得ます ── 専用の as-of スイッチはありません。合成でもステータス遷移の履歴や削除された決定は辿れません。並び替えは decided/created/number/title/status（先頭 - で降順・既定 -created）。--limit/--offset はページングします（JSON は total_matched = ページング前の件数、count = このページの件数を持ちます）。--with-body は各決定の本文を行に足します ── --filter/--limit/--offset と合成できる射影です（状態で絞ってページングする。コーパス全体を吐き出しはしません）。"),
    ("Shows a decision: body, status, the supersession chain (both directions), the premises it builds on (read those first — a premise another decision has overturned is flagged, because this decision then stands on rotten ground) and the decisions that build on it (the impact radius: overturn this one and they want revisiting), and its linked tasks. It dates itself as well — when it was recorded, when it was settled and by which facet, and when it last changed where that is neither of the two — so how fresh the record is, and whose ruling it is, come off the page rather than being guessed at. Its timeline comes with it, in the same shape task show gives its own: the count of what has been said, the newest three previewed a line each, and the way to the rest — which is where the reason an acceptance or a rejection was given lives, since --reason writes one as a comment (--json returns every comment whole, under `comments`).", "決定記録を表示します: 本文、状態、置換チェーン（両方向）、前提にしている決定（先に読むべき決定。別の決定に覆された前提には印が付きます ── その決定は腐った前提の上に立っています）、この決定を前提にする決定（影響半径: これを覆すなら見直しが要る決定）、関連タスク。 記録した日時・採択した日時と採択した facet・（そのどちらとも違うなら）最後に変わった日時も出ます ── 鮮度も、誰の裁定かも、ページから読めます。 コメント面も task show と同じ形で併せて出ます: 何件付いているか、新しい順に3件のプレビュー、残りへの導線。採択・却下の理由は --reason がコメントとして書くので、理由が読めるのはここです（--json は全件を `comments` で丸ごと返します）。"),
    ...
    ("Accepts a decision (proposed → accepted); stamps decided_at/decided_by. It is where a classification the project requires of decisions is read (the axes whose applies_to covers decisions): an axis left blank holds the acceptance and is named in the refusal. --reason records the reason for accepting as a decision comment — the reason lives on the timeline, not in a dedicated field.", "決定を採択します（proposed → accepted）。decided_at/decided_by を刻みます。プロジェクトが決定に必須としている分類（applies_to が決定に効く軸）を読む場所でもあります: 空の軸があれば採択を止め、その軸を名指します。--reason は採択の理由を決定コメントとして記録します── 理由は専用フィールドでなくタイムラインに残ります。"),
    ...
    ("Promotes a comment into a decision: the comment text becomes the body, and the project defaults to the project of what the comment sits on. What is drawn afterwards differs by kind. A task comment (AMB-TC-n) links the new decision to that task — the decision is that task's premise. A decision comment (AMB-DC-n) draws no edge: a record raised out of a decision's thread is a question that turned into its own, and a link would claim a relation this cannot know — where one holds, name it yourself with builds-on / amend / supersede. The two tables number independently, so a bare <n> naming a row in each is refused: spell the kind code. Like `decision add`, the response names any required axis the new decision is still blank on (`unmet_required_dimensions` in --json) — fill those in with `dimension set`, since there is no --dim here.", "コメントを決定記録へ昇格します: コメント本文が本文になり、プロジェクトはコメントの載っているもののプロジェクトを既定にします。その後に張る線は種別で違います。タスクのコメント（AMB-TC-n）は新しい決定をそのタスクに結びます（その決定はそのタスクの前提です）。決定記録のコメント（AMB-DC-n）は線を張りません: 決定記録のコメント欄から起きた記録は論点が切り替わったということで、結ぶと昇格が知りようのない関係を騙ることになります——本当に関係があるなら builds-on / amend / supersede で自分で名指してください。2 つの表は別々に採番するので、どちらの表にも当たる裸の <n> は拒否します: 種別コードまで綴ってください。decision add と同じく、まだ空の必須軸は応答が名指しします（--json では `unmet_required_dimensions`）── ここに --dim は無いので dimension set で埋めてください。"),
    ...
    ("Answers what version this window is at — one number, and the question a carrier plugin asks often. It moves whenever something inside the window is written and stays put when nothing is, so a carrier that remembers the last number it sent knows in one cheap call whether there is anything to send; no snapshot is built to answer it. Compare it for inequality, not for order: a `restore` winds the store back and the version comes back with it, so lower still means changed. A window nothing has written since this build began stamping answers 0. Through a project plugin's window it is that one project's version, so churn in another project sends nobody re-reading; for a human, or a plugin that reaches the device, it is the whole device's. This is a carrier's question and not a way to watch for work — for that, read `activity --since`.", "この窓がいまどのバージョンかを答えます ── 数値1つで、運ぶプラグインが繰り返し訊く問いです。窓の中に書き込みがあれば必ず動き、無ければ動かないので、最後に送った番号を覚えている運び手は、送るものがあるかどうかを1回の軽い呼び出しで判断できます。答えるためにスナップショットを組み立てることはありません。順序ではなく、等しいかどうかで比べてください: `restore` はストアを巻き戻し、バージョンもそれと一緒に戻るので、小さくなっていても「変わった」です。この build が刻み始めてから一度も書き込みの無かった窓は 0 を返します。プロジェクトのプラグインの窓から訊けばそのプロジェクト1つのバージョンなので、隣のプロジェクトが動いても誰も読み直しません。人間から訊いたとき、そして端末を読むプラグインから訊いたときは、端末全体のバージョンです。これは運び手の問いであって、作業を見張るための口ではありません ── それには `activity --since` を読んでください。"),
    ...
    ("repair fixable problems (sweep attachment rows whose record is gone, reclaim unreferenced attachment files, forget folder bindings no live project claims; all are non-destructive)", "直せる問題を修復します（付いていた先が消えた添付の行を掃き、参照の無い添付ファイルの実体を回収し、生きたプロジェクトが誰も主張しないフォルダ紐付けを索引から忘れます。どれも非破壊です）"),
    ...
    ("move the tasks classified as this value to another value of the same dimension", "この値が付いたタスクを、同じ次元の別の値へ移す"),
    ...
    ("classify at creation, resolving names as dimension set does; repeatable for different axes, and for one multi axis named several times (a single axis named twice is refused, and so is an axis that does not classify decisions). It saves the create→dimension set round trip, and a required axis filled here is one `decision accept` will not turn the acceptance away for", "作成時に分類軸の値を割り当てます。名前の解決は dimension set と同じで、軸ごとに繰り返し指定でき、multi の軸なら同じ軸を何度でも指定できます（single の軸を2度指定すると拒否。決定を分類しない軸も拒否）。作成→dimension set の往復を省き、必須の軸をここで埋めておけば decision accept が採択を断りません"),
    ("classify at creation, resolving names as dimension set does; repeatable for different axes, and for one multi axis named several times (a single axis named twice is refused). It saves the create→dimension set round trip, and what you name wins over the time-axis default", "作成時に分類軸の値を割り当てます。名前の解決は dimension set と同じで、軸ごとに繰り返し指定でき、multi の軸なら同じ軸を何度でも指定できます（single の軸を2度指定すると拒否）。作成→dimension set の往復を省き、ここで指定した値が time_axis の既定割当より優先されます"),
    ...
    ("e.g. status:accepted superseded:no dim:Theme=main", "例: status:accepted superseded:no dim:テーマ=メイン"),
    ...
    json!([
        ...
        json!({ "name": "decision add", "summary": "Records a new decision (proposed) — an append-only \"why we chose X\". A decision is a Task sibling (project-scoped), NOT a task: it has no mailbox workflow and never appears in task lists. Decisions have their own device-global number space, shown as AMB-D-N (tasks are AMB-T-N); the kind code keeps AMB-D-<n> / AMB-T-<n> unambiguous. The body should be the conclusion + rationale (compress; do not paste raw discussion, and keep PII out). The project defaults to the bound project. Classify it here with --dim: an axis the project marked required is read at `decision accept`, so one left empty is what turns the acceptance away — filling it at creation is what saves that round trip. The response names any required axis still blank (`unmet_required_dimensions` in --json), since a decision has no second stage to be turned away at and the one who accepts it is not the one who wrote it.",
            "args": [], "flags": [{ "name": "--title <str>", "required": true, "help": "decision title" },
                                   ...
                                   { "name": "--dim <axis>=<value>", "help": "classify at creation, resolving names as dimension set does; repeatable for different axes, and for one multi axis named several times (a single axis named twice is refused, and so is an axis that does not classify decisions). It saves the create→dimension set round trip, and a required axis filled here is one `decision accept` will not turn the acceptance away for" },
