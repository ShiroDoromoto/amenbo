...
impl WriteAck {
    ...
    /// Name the library action a write made, so the screen that pressed it can open it
    /// ([`crate::dto::WriteAck::actions`]).
    pub(crate) fn action(mut self, id: i64) -> WriteAck {
            ...
            .map(|w| {
                ...
                match w {
                    ...
                    Warning::Frame { theme, key, wrote, used } => SkinWarningDto {
                        ...
                        key: (*key).to_string(),
                        detail: Some(match used {
                            Some(used) => format!("'{wrote}' — drawn at {used}"),
