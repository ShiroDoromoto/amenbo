...
cleanly (no `.amenbo` ancestor to hijack the mktemp store). devtool provisions
no amenbo store of its own. (The dev GUI app is a single installed bundle, so
GUI verification is serialized across parallel sessions regardless.)
...
4. Prints an eval-able `cd` to **stdout** (diagnostics go to stderr):
...
`<repo>/../<repo-name>-worktrees` base dir once it holds no other worktree.
