...
//
// What these guard is the reading, not the pixels: **a task's span is one outline** and the step
// that takes the next task starts the next one; **depth runs down and the same depth runs across**;
// **what comes after a step is tied down the left and what it hands on down the right**; **a line
// whose two boxes are not neighbours leaves for a lane**, dashed where it goes back up; **the error
// way out is drawn only where somebody said what follows it**; and **a step nothing reaches is still
...
/**
 * A step that takes the next task — the one that begins a stretch. What makes it one is a
 * `task_take` **output** on a way out, which is where core looks for it.
 */
function taker(id: number, name: string, over: Partial<AutomationStepDto> = {}): AutomationStepDto {
...
function edge(over: Partial<AutomationEdgeDto> & { id: number; fromStepId: number }): AutomationEdgeDto {
  return { ends: "go", ...over };
}
...
const at = (picture: ReturnType<typeof layOut>, stepId: number) =>
  picture.nodes.find((one) => one.stepId === stepId)!;
...
describe("the picture of an automation", () => {
  ...
  it("puts each depth on its own row, in the order the walk reaches them", () => {
    const one = detail({
      entryStepId: 1,
      steps: [taker(1, "take"), step({ id: 2, name: "work" }), step({ id: 3, name: "check" })],
      edges: [edge({ id: 1, fromStepId: 1, toStepId: 2 }), edge({ id: 2, fromStepId: 2, toStepId: 3 })],
    });
  ...
  it("puts what is at the same depth side by side", () => {
    const one = detail({
      ...
      edges: [
        edge({ id: 1, fromStepId: 1, exitName: "yes", toStepId: 2 }),
        edge({ id: 2, fromStepId: 1, exitName: "no", toStepId: 3 }),
      ],
    });
    const picture = layOut(one);
  ...
  it("outlines the steps one task is worked by, and starts a new outline at the next taker", () => {
    const one = detail({
      entryStepId: 1,
      steps: [taker(1, "first"), step({ id: 2, name: "work" }), taker(3, "second")],
      edges: [edge({ id: 1, fromStepId: 1, toStepId: 2 }), edge({ id: 2, fromStepId: 2, toStepId: 3 })],
    });
    const picture = layOut(one);
    expect(picture.laps.map((lap) => lap.headStepId)).toEqual([1, 3]);
  ...
  it("draws no outline around steps that answer to no task, and still places what nothing reaches", () => {
    const one = detail({
      entryStepId: 1,
      steps: [step({ id: 1, name: "entry that takes nothing" }), step({ id: 2, name: "stranded" })],
    });
    const picture = layOut(one);
    expect(picture.laps).toEqual([]);
    expect(picture.nodes.map((node) => node.stepId).sort()).toEqual([1, 2]);
  });
  ...
  it("ties what comes after a step down its left and what it hands on down its right", () => {
    const one = detail({
      ...
      edges: [edge({ id: 1, fromStepId: 1, toStepId: 2 })],
      wires: [wire({ id: 1, fromStepId: 1, fromPortName: "note", toStepId: 2, toPortName: "note" })],
    });
    const picture = layOut(one);
  ...
  it("sends a line back to a shallower row out to a lane, dashed", () => {
    const one = detail({
      ...
      edges: [
        edge({ id: 1, fromStepId: 1, toStepId: 2 }),
        edge({ id: 2, fromStepId: 2, toStepId: 3 }),
        edge({ id: 3, fromStepId: 3, exitName: "again", toStepId: 2 }),
      ],
    });
    const picture = layOut(one);
  ...
  it("sends a line that skips a stretch out to a lane, and leaves it solid", () => {
    const one = detail({
      ...
      edges: [
        edge({ id: 1, fromStepId: 1, toStepId: 2 }),
        edge({ id: 2, fromStepId: 2, toStepId: 3 }),
        edge({ id: 3, fromStepId: 3, toStepId: 4 }),
        edge({ id: 4, fromStepId: 4, toStepId: 5 }),
        // Past the whole of the second stretch: forward, and no neighbour.
        edge({ id: 5, fromStepId: 2, exitName: "*", toStepId: 5 }),
      ],
    });
    const picture = layOut(one);
  ...
  it("gives two lines crossing the same rows a lane each", () => {
    const one = detail({
      ...
      edges: [
        edge({ id: 1, fromStepId: 1, exitName: "a", toStepId: 2 }),
        edge({ id: 2, fromStepId: 2, toStepId: 3 }),
        edge({ id: 3, fromStepId: 3, toStepId: 4 }),
        edge({ id: 4, fromStepId: 4, toStepId: 2 }),
        edge({ id: 5, fromStepId: 3, exitName: "*", toStepId: 2 }),
      ],
    });
    const picture = layOut(one);
  ...
  it("draws the error way out only where somebody said what follows it", () => {
    ...
    const plain = layOut(detail({ entryStepId: 1, steps, edges: [edge({ id: 1, fromStepId: 1, toStepId: 2 })] }));
    ...
    const changed = layOut(
      detail({
        ...
        edges: [
          edge({ id: 1, fromStepId: 1, toStepId: 2 }),
          edge({ id: 2, fromStepId: 1, exitName: "*", ends: "halt" }),
        ],
      }),
    );
  ...
  it("hangs a way out that names no step below the step it leaves, and marks how it ends", () => {
    const picture = layOut(
      detail({
        entryStepId: 1,
        steps: [taker(1, "take")],
        edges: [edge({ id: 1, fromStepId: 1, ends: "done" })],
      }),
    );
  ...
  it("puts a + on every edge and on no wire", () => {
    const picture = layOut(
      detail({
        ...
        edges: [edge({ id: 1, fromStepId: 1, toStepId: 2 }), edge({ id: 2, fromStepId: 2, ends: "done" })],
        wires: [wire({ id: 1, fromStepId: 1, fromPortName: "note", toStepId: 2, toPortName: "note" })],
      }),
    );
  ...
  it("names the required inputs nothing reaches, and says nothing about the ones that are fed", () => {
    ...
    expect(at(picture, 2).action).toBe("Review");
