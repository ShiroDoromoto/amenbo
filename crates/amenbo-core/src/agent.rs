...
fn capabilities() -> Value {
    let caps = vec![
        ...
        cap(
            "Move a decision through its lifecycle (finish-writing / accept / reject / reopen / edit / supersede / delete)",
            &["decision finish-writing", "decision accept", "decision reject", "decision reopen", "decision edit", "decision supersede", "decision delete"],
        ),
...
fn all_commands() -> Value {
    json!([
        ...
        json!({ "name": "decision add", "summary": "Records a new decision (proposed) — an append-only \"why we chose X\". A decision is a Task sibling (project-scoped), NOT a task: it has no mailbox workflow and never appears in task lists. Decisions have their own device-global number space, shown as AMB-D-N (tasks are AMB-T-N); the kind code keeps AMB-D-<n> / AMB-T-<n> unambiguous. The body should be the conclusion + rationale (compress; do not paste raw discussion, and keep PII out). The project defaults to the bound project. Classify it here with --dim: nothing is demanded at the record (`AMB-D-925`) — the axes a project marked required are read one stage on, at `decision finish-writing`, which will not end the writing until they are answered. A record left blank on one goes through, and the response names what is still to classify (`unmet_required_dimensions` in --json); fill it in with `dimension set`, or save the round trip by naming it here. The time axis is filled from the era that contains today, so it is rarely among them.",
        ...
        cmd("decision list", "Lists decisions (status:proposed|accepted|rejected, draft:yes|no — whether the writing is still unfinished, the premise that holds a task resting on it out of ready, superseded:yes|no — the edge itself, so superseded:yes lists the decisions another decision draws a supersedes edge at — project:, dim:<axis>=<value> and its time_axis:<value> sugar, which read the same axes the tasks are classified on and fold the same way (different axes AND, the same axis ORs), decided_before:/decided_after: over the day a decision was accepted (YYYY-MM-DD, or today/-30d; both ends inclusive; a decision that was never accepted has no such day and matches neither)). Words are not among the keys — search <word> --kind decision is what finds where they are written. To ask which policies were settled by a date, compose this filter with superseded: — there is no separate as-of switch, and the composition recovers neither status transitions nor deleted decisions. Sort by decided/created/number/title/status (prefix - for descending; default -created). --limit/--offset page (JSON carries total_matched = the count before paging, count = this page). --with-body adds each decision's body to the rows — a projection that composes with --filter/--limit/--offset (narrow by status and page; it does not dump the whole corpus).",
            json!([{ "name": "--project <id>", "help": "limit to the given project (human only)" },
                   { "name": "--filter <expr>", "help": "e.g. status:accepted draft:no superseded:no dim:Theme=main" },
