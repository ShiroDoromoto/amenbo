...
const JA_PHRASEBOOK: &[(&str, &str)] = &[
    ...
    ("Hand a carrier plugin what it takes this store's data outward with — the version of what it may see, one whole snapshot of it, and what has changed since a cursor (a carrier's road, not a way to read the work)", "運ぶプラグインが、このストアのデータを外へ持ち出すのに使うものを渡す ── 見てよい範囲の版と、その全体のスナップショット1枚と、カーソルから先の変更（運ぶための道であって、作業を読むための道ではない）"),
...
fn capabilities() -> Value {
    let caps = vec![
        ...
        cap(
            "Hand a carrier plugin what it takes this store's data outward with — the version of what it may see, one whole snapshot of it, and what has changed since a cursor (a carrier's road, not a way to read the work)",
            &["sync version", "sync snapshot", "sync changes"],
        ),
        cap(
