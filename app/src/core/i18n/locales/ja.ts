...
export const ja: Translation = {
  ...
  ui: {
    ...
    "auto.place.ownedWhat": "プロンプトはアクションの持ちもの。直すのはアクションのビルド画面で、そこで直すと、このアクションを置いたほかの配置にも効く",
    "auto.place.agents": "ステップごとに、誰にやらせるか",
    "auto.place.agentsWhat": "この配置だけの選択。同じアクションをほかに置いても、そこでは別に選ぶ",
    ...
    "auto.step.nextGroupExit": "このアクションの出力で終える",
    ...
    "auto.step.taskNotes": "タスクの本文を渡す",
    "auto.step.taskDecisions": "リンクした決定を渡す",
    "auto.step.taskComments": "コメントを渡す",
    "auto.step.ready": "前提",
    ...
    "auto.lib.builtin": "組み込み（Amenbo が用意する）",
    "auto.builtin.readOnly": "組み込みのアクションは Amenbo が用意するもので、直せません。ここでは読むだけです。",
    "auto.builtin.does": "何をするか",
  ...
  err: {
    ...
    not_ready_automation_task_left_open: "{step}：終了条件 {exit} から、タスクを閉じないまま {to} へ進んで次のタスクを取る",
    not_ready_automation_task_left_open_unnamed: "{step}：タスクを閉じないまま {to} へ進んで次のタスクを取る",
    not_ready_automation_task_left_open_at_end: "{step}：終了条件 {exit} から、タスクを閉じないまま実行が終わる",
    not_ready_automation_task_left_open_at_end_unnamed: "{step}：タスクを閉じないまま実行が終わる",
