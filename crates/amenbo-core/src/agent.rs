...
const JA_PHRASEBOOK: &[(&str, &str)] = &[
    ...
    ("Shows a dimension: name, its readable key (slug), notes, kind (single or multi, ordered, time-axis, show-on-card, required, and the one side it is narrowed to, if it is), and its values with their own keys.", "次元を表示します: 名前、読める鍵（slug）、メモ、種類（single か multi、順序付き、時間軸、カードに出す、必須、そして片側に絞ってあればその側）、そして値とその鍵。"),
    ...
    ("Updates a dimension's name, notes, how many of its values one record may hold, value ordering, time-axis role, whether it goes on the task card, whether it must be answered, which side of the store it classifies, and/or its readable key. Only the given fields change.", "次元の名前・メモ・1件のレコードが持てる値の数・値の順序・時間軸の役割・タスクカードに出すかどうか・答えを必須にするかどうか・ストアのどちら側に効くか・読める鍵を更新します。指定したフィールドだけが変わります。"),
...
fn all_commands() -> Value {
    json!([
        ...
        json!({ "name": "dimension show", "summary": "Shows a dimension: name, its readable key (slug), notes, kind (single or multi, ordered, time-axis, show-on-card, required, and the one side it is narrowed to, if it is), and its values with their own keys.",
            "args": [{ "name": "id", "required": true, "help": "dimension id, slug or name" }],
            "flags": [{ "name": "--json", "help": "machine-readable output" }],
            "examples": ["amenbo dimension show Category --json"] }),
        json!({ "name": "dimension update", "summary": "Updates a dimension's name, notes, how many of its values one record may hold, value ordering, time-axis role, whether it goes on the task card, whether it must be answered, which side of the store it classifies, and/or its readable key. Only the given fields change.",
            "args": [{ "name": "id", "required": true, "help": "dimension id, slug or name" }],
        ...
        json!({ "name": "dimension value-rm", "summary": "Deletes a dimension value permanently; its task assignments go with it unless --reassign-to moves them to another value of the same axis (alias: value-delete). A required axis refuses to empty tasks out: with tasks classified as the value, --reassign-to is required, and its last value is refused outright — a requirement nobody can answer would stop every creation on the project, so lower it first.",
            "args": [{ "name": "dimension", "required": true, "help": "dimension id, slug or name" },
                     { "name": "value", "required": true, "help": "value id, slug or name (within the dimension)" }],
