...
impl Store {
    ...
    /// **What one of a step's declared outputs carries**, by its id — `None` where the step declares
    /// no output of that id.
    ///
    /// What a port takes is what decides which command hands it on, so this is asked before anything is
    ...
    pub fn automation_out_kind(
        ...
        crate::ops::automation_report::out_kind(self.engine.conn(), run_step_id, port_id)
    }
}
