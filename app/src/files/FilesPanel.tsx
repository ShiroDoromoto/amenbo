...
import type {
  FolderAppDto, FolderCarriedDto, FolderChangesDto, FolderEntryDto, FolderFileDto,
...
import {
  folderEntries, folderImport, folderOpenFile, folderOpenFileWith, folderOpenWith, folderRead,
...
export function FilesPanel({ projectId, onOpenLedger, show, tab, onTab, onClose }: {
  ...
  //
  // **What arrived is not said, and what did not is.** The folder is watched, so a file that came in
  // is about to be listed as one that changed lately — a line saying so would be a second, slower
  // account of what the panel is already showing. A carry that stopped has nothing drawing it, and
  // that is what the toast is for.
  useEffect(() => {
