...
const JA_PHRASEBOOK: &[(&str, &str)] = &[
    ...
    ("Hand a carrier plugin what it takes this store's data outward with — the version of what it may see, and one whole snapshot of it (a carrier's road, not a way to read the work)", "運ぶプラグインが、このストアのデータを外へ持ち出すのに使うものを渡す ── 見てよい範囲の版と、その全体のスナップショット1枚（運ぶための道であって、作業を読むための道ではない）"),
...
fn capabilities() -> Value {
    let caps = vec![
        ...
        cap(
            "Hand a carrier plugin what it takes this store's data outward with — the version of what it may see, and one whole snapshot of it (a carrier's road, not a way to read the work)",
            &["sync version", "sync snapshot"],
        ),
        cap(
...
fn all_commands() -> Value {
    json!([
        ...
        cmd("sync version", "Answers what version this window is at — one number, and the question a carrier plugin asks often. It moves whenever something inside the window is written and stays put when nothing is, so a carrier that remembers the last number it sent knows in one cheap call whether there is anything to send; no snapshot is built to answer it. Compare it for inequality, not for order: a `restore` winds the store back and the version comes back with it, so lower still means changed. A window nothing has written since this build began stamping answers 0. Through a plugin's window it is that one project's version, so churn in another project sends nobody re-reading; for a human it is the whole device's. This is a carrier's question and not a way to watch for work — for that, read `activity --since`.",
            json!([{ "name": "--json", "help": "machine-readable output" }]),
