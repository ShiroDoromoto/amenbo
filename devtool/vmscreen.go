...
func takeScreenLock(path, label string) (func(), error) {
	...
	// Written once the lock is held, and the file emptied again on the way out, so what a refused
	// caller reads is the holder standing now and never the one before it.
	_ = f.Truncate(0)
