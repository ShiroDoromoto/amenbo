...
fn all_commands() -> Value {
    json!([
        ...
        json!({ "name": "plugin update", "summary": "Reports which installed plugins the catalog holds a different build of. Detection is the catalog amenbo already fetches whole laid beside the manifest that sits next to each installed binary — no central server, no per-plugin request. A manifest carries no version number, so what is compared is the checksum of this machine's asset: the digest of the exact bytes that would run here, and so the build's identity — two entries with the same digest are the same executable however the description around them was rewritten. It therefore reports different, not newer: a catalog that rolls an entry back offers that older build, because the catalog is the authority on what is published. A plugin the catalog does not list is passed over rather than reported (installed by hand, or delisted). Cheap on purpose: with nothing installed no catalog is read at all, and otherwise a cached catalog younger than an hour answers with no request — which is what lets a check ride along with something you were doing anyway instead of needing a timer. Applying an update is a separate act, so --check is required today.",
