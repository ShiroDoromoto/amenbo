...
// window named, `shot` shoots it, and `find` / `click-named` / `set-date` read, press and write
// inside it and nowhere else. Left out, it means the app's one window — and an app with two is told to say which rather
...
guard args.count >= 2 else {
    fail("usage: screen <front|shot|read|find|click-named|click|dblclick|type|key|set-date|trusted> … [--window <title>]")
}
