...
const JA_PHRASEBOOK: &[(&str, &str)] = &[
    ...
    ("Hand the human the text that wires this folder's AI to run `agent` at every session start (amenbo writes no settings file — they paste it)", "このフォルダの AI がセッション開始のたびに `agent` を走らせるための設定を、人間に渡します（amenbo は設定ファイルを書きません——貼るのは人間です）"),
    ("Prints the whole settings file for one AI tool to paste, so that tool runs `amenbo agent` at the start of every session — the reach the managed block in CLAUDE.md/AGENTS.md cannot have, since it works only if the file is read. The catalog covers the paste-only providers: claude-code, github-copilot, cursor, codex-cli, gemini-cli (the argument refuses anything else, naming what it takes). What the snippet injects is the launch instruction — the one line that says to run `agent --json` and follow it — never the spec itself, which the agent holding the instruction fetches for itself. **amenbo writes no settings file**: this hands over text, the human pastes it, and that is the whole division. So this is what you hand a human who asks how to stop their AI from missing amenbo — with the file it goes in, which the answer names. stdout is the paste and nothing else (it pipes to a clipboard or a file); where it goes is said on stderr, and `--json` carries both. Opens no store and needs no bound folder — a fresh checkout can be wired before anything else. It reports nothing about whether this folder is already wired: it is the handing-over face, not the state.", "その AI ツールがセッション開始のたびに `amenbo agent` を走らせるよう、貼るべき設定ファイルの全文を出力します——CLAUDE.md/AGENTS.md の managed block には届かない到達性です（block はファイルが読まれて初めて効くため）。カタログが覆うのは「貼るだけで済む」プロバイダー: claude-code / github-copilot / cursor / codex-cli / gemini-cli（それ以外は引数の段で、受け付ける名前を挙げて拒否します）。スニペットが注入するのは**起動指示**——`agent --json` を実行してそれに従えと言う1行——だけで、仕様そのものではありません（指示を受けた AI が自分で取りに行きます）。**amenbo は設定ファイルを書きません**: 渡すのはテキストで、貼るのは人間、分担はそれだけです。ですから「AI が amenbo を取りこぼさないようにするには」と訊かれた人間に渡すのはこれで、貼り先のファイルもこの出力が名指しします。stdout は貼るテキストだけです（クリップボードやファイルへパイプできます）。貼り先は stderr に出し、`--json` は両方を運びます。ストアを開かず bind も要りません——まだ何も bind していないチェックアウトでも配線できます。このフォルダが既に配線済みかは何も報告しません: これは渡す面で、状態の面ではありません。"),
    ...
    ("put it on this machine's clipboard instead of printing it (refused, naming the pipe, where the machine has no clipboard tool)", "出力せず、この端末のクリップボードへ載せます（クリップボードの道具が無い端末では、パイプの書き方を示して拒否します）"),
    ("machine-readable output (tool / label / paste_into / snippet / copied)", "機械可読な出力（tool / label / paste_into / snippet / copied）"),
...
fn capabilities() -> Value {
    let caps = vec![
        ...
        cap(
            "Hand the human the text that wires this folder's AI to run `agent` at every session start (amenbo writes no settings file — they paste it)",
            &["agent-hook snippet"],
        ),
        cap(
...
fn all_commands() -> Value {
    json!([
        ...
        json!({ "name": "agent-hook snippet", "summary": "Prints the whole settings file for one AI tool to paste, so that tool runs `amenbo agent` at the start of every session — the reach the managed block in CLAUDE.md/AGENTS.md cannot have, since it works only if the file is read. The catalog covers the paste-only providers: claude-code, github-copilot, cursor, codex-cli, gemini-cli (the argument refuses anything else, naming what it takes). What the snippet injects is the launch instruction — the one line that says to run `agent --json` and follow it — never the spec itself, which the agent holding the instruction fetches for itself. **amenbo writes no settings file**: this hands over text, the human pastes it, and that is the whole division. So this is what you hand a human who asks how to stop their AI from missing amenbo — with the file it goes in, which the answer names. stdout is the paste and nothing else (it pipes to a clipboard or a file); where it goes is said on stderr, and `--json` carries both. Opens no store and needs no bound folder — a fresh checkout can be wired before anything else. It reports nothing about whether this folder is already wired: it is the handing-over face, not the state.",
            ...
            "flags": [{ "name": "--copy", "help": "put it on this machine's clipboard instead of printing it (refused, naming the pipe, where the machine has no clipboard tool)" },
                      { "name": "--json", "help": "machine-readable output (tool / label / paste_into / snippet / copied)" }],
            "examples": ["amenbo agent-hook snippet claude-code", "amenbo agent-hook snippet cursor --json", "amenbo agent-hook snippet codex-cli > .codex/hooks.json"] }),
