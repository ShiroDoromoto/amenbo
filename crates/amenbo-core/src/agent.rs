...
fn all_commands() -> Value {
    json!([
        ...
        json!({ "name": "skin list", "summary": "Every skin that can be worn here, and which of them is on. Four ship with the build (high-contrast, washi, terminal, retro) and are listed first; the rest are the device's own, kept one file per skin at `<base>/skins/<name>.zip` (a zip holding `skin.yaml` and the skin's materials) or `<base>/skins/<name>.yaml` (the document on its own). The one that is on is a name in config.json (`config set skin`), never synced, because it points at this machine. A file in the directory that will not read is listed as unreadable rather than left out. The four shipped names are reserved: no file may be taken in under one, and none of them is on the device to remove.",
            "args": [], "flags": [{ "name": "--json", "help": "machine-readable output" }],
            "examples": ["amenbo skin list --json"] }),
        json!({ "name": "skin add", "summary": "Takes a skin file in, to `<base>/skins/<name><ext>` under the name the file gives itself, in the shape it arrived in and byte for byte. Two shapes: a zip holding `skin.yaml` at its root beside the skin's materials, and the document on its own. It is read, checked and measured first. A zip is turned away whole where an entry's name points outside it (`../`, or an absolute path), where one file unpacks past 8MB, where the whole unpacks past 32MB — counted while unpacking, so an index that lies does not get past — and where there is no `skin.yaml` in it. One name is one file: taking a skin in under a name already held in the other shape replaces it. A key this build does not know, a name it keeps to itself, a value that is not text, and a value that is text but not a shape a declaration can hold (empty, over 512 characters, or carrying `;` `{` `}` `<` `>` `\\`, a newline, a comment delimiter or `url(`) are dropped with a warning naming each and why; a later `skin_v`, a `themes` line that disagrees with the tables, a name that cannot be a filename, and an embedded font with no `license_text` turn the whole file away. A skin may carry `titles:`, the name per language (language code to the name), which the window shows a reader in their own language and falls back from to `title`; the CLI stays on `title`. A skin may carry one font (`font_file`: family/format/file/license/license_text), the face being a file in the zip and `file` its name there; it is taken only as woff2 under 2MB, and anything else about it — a name nothing in the zip answers to included — drops the font with a warning and keeps the colours. The four ladders (text, spacing, radius, shadow) are not written out name by name — a skin sets `fs-scale` / `s-scale` / `r-scale` / `shadow-scale` and amenbo multiplies its own values. The ceiling is 1.30 on the first three (measured); the floor is 0.85 on the type and the spacing, and 0 on the radius and the shadow, which have no size that stops them working. A number outside that is brought inside and both values reported; only text that is no number is dropped. The frame pair is held to what this build draws: `border-style` takes solid/double/none, `border-w` is put back inside 0-4px, and a `double` frame is drawn at 3px whatever was asked for (the widths that split are not one range). `font-smooth` is held the same way and takes antialiased/auto/none; a word outside those is dropped. A colour pairing under WCAG AA is reported with its number and does NOT stop the import — an author has to be able to try work in progress. A name already held is refused with both versions named, unless --yes replaces it; one of the four shipped names is refused outright, which --yes does not answer.",
            ...
            "examples": ["amenbo skin add ./mine.amenbo-skin.yaml", "amenbo skin add ./mine.yaml --yes"] }),
        ...
        json!({ "name": "skin validate", "summary": "Runs the reading, the check and the contrast measure over a file — a zip or a bare document, the same two `skin add` takes — without taking it in — the author's face of what `skin add` does, so the reason a file will be turned away is learned before it is handed to anybody. Exits non-zero on a refusal; warnings and short pairings are reported and exit 0.",
            "args": [{ "name": "path", "required": true, "help": "the .yaml to read" }],
            "flags": [{ "name": "--json", "help": "machine-readable output" }],
            "examples": ["amenbo skin validate ./mine.yaml --json"] }),
