...
fn capabilities() -> Value {
    let caps = vec![
        ...
        cap(
            "Find where a word is written (tasks, decisions, the comments on both, an automation's shared documents)",
            &["search"],
        ),
        cap(
        ...
        cap(
            "Build an automation — the library of prompts, the steps made of them, the documents they share, and the settings each one answers",
            &[
                "automation add", "automation update", "automation rm",
                "automation action add", "automation action update", "automation action rm",
                ...
                "automation note add", "automation note update", "automation note rm",
                "automation note link", "automation note unlink",
            ],
        ),
        cap(
            "Read an automation back — a project's automations, one definition in full, the library it reaches, and one action in full",
        ...
        cap(
            ...
            &["automation take", "automation out", "automation done"],
        ),
        cap(
    ...
    json!([
        ...
        json!({ "name": "search", "summary": "Finds where words are written, across tasks, decisions and the comments on both — plus the labels a task is filed under, the names of what is attached, and the documents an automation's steps share. Answers with one line per PLACE, not per record: the face the words landed on (title / body / comment / label / attachment), the record it belongs to (AMB-T-<n> / AMB-D-<n> / AMB-AUT-<n>), the comment to open where there is one, and a short excerpt. Words are ANDed and match as substrings, with no word boundaries (so a search for part of a compound word finds it); full-width, case and kana spellings are folded together. Every word has to land somewhere on the record — not all on one face — and each face carrying one is a line, which is what makes the answer 'here is where each of your words is written'. A word written as a ref (AMB-T-<n> / AMB-D-<n>) pins that record to the top, so holding a number takes the same command as holding a phrase. A number alone (12 / #12) is a ref as well: the two sides number themselves apart, so it pins the task AND the decision that carry it, and --kind keeps the side you named. Default order is the face first (a name outranks a paragraph, and that a remark), newest within it. Every line also says where the record it points at stands — a task's status, its priority and what it is filed under, a decision's status — so a hit on work that is already over says so without a second read. This read has a ceiling of its own — a hit carries an excerpt — so with no --limit it returns the first 20 and reports the total.",
            ...
            "flags": [{ "name": "--project <id>", "help": "look in this project alone (human only — an AI is already scoped to its bound project). An argument of its own, not a --filter key: a project is an axis tasks and decisions carry alike, so scoping to one keeps decisions in the answer" },
                      { "name": "--filter <expr>", "help": "narrow structurally, in the grammar of the side --kind names: task list's for a task (see filterGrammar), decision list's for a decision. It requires --kind, because the two grammars share spellings that mean different things — status:rejected is work decided against on one side and a decision turned down on the other — and --kind automation takes none, an automation having no listing to take a grammar from" },
                      { "name": "--kind <task|decision|automation>", "help": "keep one side: the words on a task, on a decision, or in the documents an automation's steps share (`AMB-D-944`). The automation side has one face — its own name, notes and preamble are not indexed, and what a run did is reached from the task it worked on rather than searched for" },
                      { "name": "--face <title|body|comment|label|attachment>", "help": "keep one face of the record: its name, its text (a shared document's body included), a remark on it, a label it is filed under, the name of what is attached. The axis beside --kind and judged apart from it, so naming both is the product of the two — --kind decision --face comment is the remarks on decisions, which neither axis alone can ask for" },
            ...
            "examples": ["amenbo search notification target --json", "amenbo search AMB-T-<n> --json", "amenbo search rollout --kind decision --json", "amenbo search rollout --kind decision --face comment --json", "amenbo search coding standards --kind automation --json", "amenbo search notification --kind task --filter \"status:todo\" --limit 5 --json", "amenbo search rollout --kind decision --filter \"status:decided\" --json"] }),
        ...
        // Automations — the building side. Every part is named by the id its `add` printed: they carry
        // no conversational ref, being named by the automation they sit in rather than by a number
        // anybody types back. Nothing here refuses an unfinished automation; the launch check does.
        cmd("automation add", "Creates an automation: a name, what it is for, and the preamble prepended to every step's launch. Left out, --preamble puts the standing operating rules there — what every step is told before its own prompt — and an empty string is how an automation carries none. It starts with no steps and no entry, which is not refused here: the launch check is where an unfinished one is caught. Prints the id the step and note commands take.",
            json!([{ "name": "--name <str>", "help": "what this automation is called", "required": true },
                   ...
                   { "name": "--preamble <text>", "help": "the text prepended to every step's launch; left out, the standing operating rules go in" },
        ...
        cmd("automation update", "Changes an automation's name, notes, preamble, or whether it is archived. Only the fields given change.",
            json!([{ "name": "<id>", "help": "automation id", "required": true },
                   { "name": "--name <str>", "help": "rename it" },
                   { "name": "--notes <text>", "help": "what it is for (`-` reads stdin)" },
                   { "name": "--preamble <text>", "help": "the text prepended to every step's launch (`-` reads stdin)" },
                   { "name": "--archived <true|false>", "help": "whether it is archived" }]),
            json!(["amenbo automation update 3 --preamble \"\"", "amenbo automation update 3 --archived true"])),
        cmd("automation rm", "Deletes an automation with every step, way out, port, setting, edge, wire and shared document built into it. It rides one transaction, so there is no half-deleted picture. Confirms unless --yes.",
            json!([{ "name": "<id>", "help": "automation id", "required": true },
                   { "name": "--yes/-y", "help": "skip the confirmation" }]),
            json!(["amenbo automation rm 3 --yes"])),
        cmd("automation list", "The automations of one project, in the order they were placed in: the id, the name, how many steps it is built out of, and whether it is archived. Archived ones are listed too — archiving keeps one out of the way rather than removing it, and a listing that hid them would leave an id nothing explains.",
            json!([{ "name": "--project <name|id>", "help": "project (defaults to the bound project)" },
                   { "name": "--json", "help": "machine-readable output" }]),
            json!(["amenbo automation list --json"])),
        cmd("automation show", "One automation in full, which is the only way to read a definition back from a terminal. It prints the automation's own notes and preamble, then every step with what it runs under — the prompt it runs on (its own, or the library action's, resolved the same way a run resolves it), what it takes in, what it is set to, each way out with what leaving through it hands on, what happens after each one is taken, and what is wired from it to a later step — then the documents its steps share. Prompts and document bodies are printed whole: a definition is read back to check what was built, and a snippet would send the reader to a screen to finish the sentence. An edge hanging on a way out the step no longer declares is written out as such, that being why a picture stops walking.",
            json!([{ "name": "<id>", "help": "automation id", "required": true },
                   { "name": "--json", "help": "machine-readable output" }]),
            json!(["amenbo automation show 3", "amenbo automation show 3 --json"])),
        cmd("automation entry set", "Names the step a run starts at, or clears it with --clear. From it the edges are walked, so the place every other step sits in the picture falls out of this one answer. An automation with no entry saves; launching one is refused at the launch check.",
            json!([{ "name": "<id>", "help": "automation id", "required": true },
                   { "name": "--step <id>", "help": "the step to start at" },
                   { "name": "--clear", "help": "leave it starting nowhere" }]),
            json!(["amenbo automation entry set 3 --step 11", "amenbo automation entry set 3 --clear"])),

        cmd("automation start", "Starts an automation: checks it, copies its steps into the run so later edits cannot change what is under way, and takes a lane if one is free (queued if not). It takes nothing else — which tasks a step works on and which folder it runs in are the automation's own answers, given while it was built. An unfinished one is refused as not_ready_automation (a code of its own, apart from a reservation's not_ready), naming every reason: a way out with nothing after it, a required input nothing reaches, a required setting nobody answered, an agent this machine cannot start, no steps, no entry, an entry that takes no task. A model the agent does not have here is NOT among them from a terminal: knowing costs a login shell and that provider starting up, and the answers the app keeps are in the app's own process — so the GUI's launch place names it and a step started from here meets it in the pane instead. An archived one is refused as invalid_automation_archived — bring it back with `automation update <id> --archived false`. Prints the run id that pause / resume / stop take.",
            json!([{ "name": "<id>", "help": "automation id", "required": true },
                   { "name": "--json", "help": "machine-readable output" }]),
            json!(["amenbo automation start 3 --actor ai"])),
        cmd("automation pause", "Asks a run to pause. A step under way finishes first and the run pauses at the end of it, keeping the task it is working and giving the lane back; a run that is only queued pauses now. `resume` picks it up from the same way out.",
            json!([{ "name": "<run>", "help": "run id", "required": true },
                   { "name": "--json", "help": "machine-readable output" }]),
            json!(["amenbo automation pause 7 --actor ai"])),
        cmd("automation resume", "Picks a paused run up again, from the way out the step before it left through. With every lane held it waits its turn instead, the same queue a start joins.",
            json!([{ "name": "<run>", "help": "run id", "required": true },
                   { "name": "--json", "help": "machine-readable output" }]),
            json!(["amenbo automation resume 7 --actor ai"])),
        cmd("automation stop", "Stops a run: hands the task it was working back to todo, gives up the lane, and leaves a comment on that task saying how far it got. Unlike pause it does not wait for the step under way — use it where a pause will not land.",
            json!([{ "name": "<run>", "help": "run id", "required": true },
                   { "name": "--json", "help": "machine-readable output" }]),
            json!(["amenbo automation stop 7 --actor ai"])),

        cmd("automation take", "Takes the task this stretch of the run is about: reserves it and declares it in one act, so no window exists where it is held by nobody the store can name. It succeeds only from todo, and is refused with already_reserved otherwise — the same compare-and-swap every reservation goes through. Typed inside the terminal a run opened for a step, which is where the step it speaks for is read from; outside one it is refused.",
            json!([{ "name": "<task>", "help": "the task to take (AMB-T-n)", "required": true },
                   { "name": "--json", "help": "machine-readable output" }]),
            json!(["amenbo automation take AMB-T-812 --actor ai"])),
        cmd("automation out", "Puts down one thing this step produced, under the name its port was declared with. Written <name>=<value>, or <name> --file <path> for a file, which is attached to this step execution first. WHAT THE NAME WAS DECLARED TO CARRY DECIDES HOW THE WORDS ARE READ: on a value port they are the answer, and on a task_make port they name a task this step raised along the way, which is written as the task. The task the run is about does not come this way and is refused here, naming `automation take` — reserving it and declaring it are one act. A name the step did not declare is refused, and so is a payload of the wrong kind. Putting the same name down twice replaces the first. Which way out it belongs to is stamped at `done`, not here.",
            json!([{ "name": "<name>=<value>", "help": "what is handed on, or just <name> beside --file", "required": true },
                   { "name": "--file <path>", "help": "a file to hand on, instead of a value" },
                   { "name": "--json", "help": "machine-readable output" }]),
            json!(["amenbo automation out note=\"nothing to fix\" --actor ai",
                   "amenbo automation out report --file ./report.md --actor ai",
                   "amenbo automation out \"a task to redo it\"=AMB-T-9001 --actor ai"])),
        cmd("automation done", "Says this step is finished: which way out it took, and what it did. The run reads the way out to decide what happens next — another step, the run closing, or the run stopping for a person. --exit left out is the unnamed way out, and a name the step did not declare is taken as the error one, the name kept in the record. A missing required output is refused, named, before anything is written. --out repeats for anything not already put down with `automation out`. Prints what the run does next.",
            json!([{ "name": "--report <text>", "help": "what this step did (`-` reads stdin)", "required": true },
                   { "name": "--exit <name>", "help": "the way out taken, as the step declared it" },
                   { "name": "--out <name>=<value>", "help": "one more thing produced — repeat for several" },
                   { "name": "--json", "help": "machine-readable output" }]),
            json!(["amenbo automation done --report - --exit \"something to fix\" --actor ai"])),

        cmd("automation action add", "Adds a prompt to the library — one worth using twice. It names no agent and no model: who carries it out is the step's answer, so two automations can run the same action with different agents. It is born carrying the unnamed way out and the error one (`*`). --global puts it in the device's library, which every project on this machine reaches; that shelf is a human's to write, and an AI bound to a project is turned away from it.",
            json!([{ "name": "--name <str>", "help": "what this action is called", "required": true },
                   { "name": "--prompt <text>", "help": "the prompt itself (`-` reads stdin)", "required": true },
            ...
            json!(["amenbo automation action add --name \"Write the tests\" --prompt -"])),
        cmd("automation action list", "The library a project reaches: the device's shelf first, then the project's own, each row with the id, the name, whether it is the device's, and how many automations run it. The two answer as one list because a step here may be pointed at either. --global narrows it to the device's shelf rather than opening a second place to look. The count is of automations, not steps: two steps of one automation running the same action is one automation whose runs change when the prompt is rewritten.",
            json!([{ "name": "--project <name|id>", "help": "project (defaults to the bound project)" },
                   { "name": "--global", "help": "the device's library alone" },
                   { "name": "--json", "help": "machine-readable output" }]),
            json!(["amenbo automation action list --json", "amenbo automation action list --global"])),
        cmd("automation action show", "One library action in full: which shelf holds it, how many automations run it, its whole prompt, and what it declares for every step pointing at it — the ways out, what each hands on, what it takes in, and the settings a step using it has to answer. An action's settings rows are the declaration alone; the answer written while building sits on the step, and `automation show` is where the two are read together.",
            json!([{ "name": "<id>", "help": "action id", "required": true },
                   { "name": "--json", "help": "machine-readable output" }]),
            json!(["amenbo automation action show 7"])),
        cmd("automation action update", "Renames a library action, or rewrites its prompt. Only the fields given change. A step pointing at it reads the new text on its next run.",
            json!([{ "name": "<id>", "help": "action id", "required": true },
                   { "name": "--name <str>", "help": "rename it" },
                   { "name": "--prompt <text>", "help": "rewrite the prompt (`-` reads stdin)" }]),
            json!(["amenbo automation action update 7 --prompt -"])),
        cmd("automation action rm", "Deletes a library action with the ways out, ports and settings it declared. Refused while a step runs it — the step would be left reading declarations that are gone. Confirms unless --yes.",
            json!([{ "name": "<id>", "help": "action id", "required": true },
                   { "name": "--yes/-y", "help": "skip the confirmation" }]),
            json!(["amenbo automation action rm 7 --yes"])),

        cmd("automation step add", "Adds a step to an automation. It either runs a library action (--action) or carries a prompt of its own (--prompt), and which it is decides where its ways out, settings and inputs are read from: the action's, or its own. A step with its own prompt is born carrying the unnamed way out and the error one; one pointing at an action declares none. --work-dir names the setting or the input the working folder is taken from — a name, not a path. Prints the id the edge, wire and setting commands take.",
            json!([{ "name": "<automation>", "help": "automation id", "required": true },
                   { "name": "--name <str>", "help": "what this step is called", "required": true },
                   { "name": "--agent <str>", "help": "who is asked to carry it out", "required": true },
                   { "name": "--action <id>", "help": "run this library action" },
                   { "name": "--prompt <text>", "help": "the prompt written for this step alone (`-` reads stdin)" },
                   { "name": "--model <str>", "help": "which model; left out, the agent's own default stands" },
                   { "name": "--interactive", "help": "let it wait for a person (it then holds a lane while nobody answers)" },
                   { "name": "--work-dir <name>", "help": "the setting or input the working folder is taken from" },
                   { "name": "--report-to-task", "help": "also land its report as a comment on the task" },
                   { "name": "--no-history", "help": "do not hand it the run's story so far" }]),
            json!(["amenbo automation step add 3 --name \"Review\" --action 7 --agent claude",
                   "amenbo automation step add 3 --name \"Fix\" --prompt - --agent claude --work-dir folder"])),
        cmd("automation step update", "Changes a step. Only the fields given change. Switching where its prompt comes from takes its declarations with it — a step pointed at an action loses the ways out, ports and settings it declared, and one given its own prompt is born with the two ways out. What is carried over neither way is the wiring: an edge or a wire naming a way out that is no longer declared stops resolving, visibly, rather than being rewritten underneath.",
            json!([{ "name": "<id>", "help": "step id", "required": true },
                   { "name": "--name <str>", "help": "rename it" },
                   { "name": "--action <id>", "help": "run this library action instead" },
                   { "name": "--prompt <text>", "help": "carry this prompt instead (`-` reads stdin)" },
                   { "name": "--agent <str>", "help": "who is asked to carry it out" },
                   { "name": "--model <str> / --clear-model", "help": "which model, or the agent's own default" },
                   ...
                   { "name": "--report-to-task <true|false>", "help": "whether its report lands as a comment on the task" },
                   { "name": "--history <true|false>", "help": "whether it is handed the run's story so far" }]),
            json!(["amenbo automation step update 11 --agent codex --model gpt-5"])),
        cmd("automation step rm", "Deletes a step with its declarations, the documents handed to it, and every edge and wire naming it. Confirms unless --yes.",
            json!([{ "name": "<id>", "help": "step id", "required": true },
                   { "name": "--yes/-y", "help": "skip the confirmation" }]),
            json!(["amenbo automation step rm 11 --yes"])),

        cmd("automation exit add", "Declares a way out of a step or a library action — the whole condition the next step is chosen by. Both are born carrying the unnamed way out and the error one (`*`), so this is for the second and every one after it. `*` is refused as a name: every declarer carries it already. A step that runs a library action declares nothing of its own — write it on the action.",
        ...
        cmd("automation exit rename", "Renames a way out, or makes it the unnamed one with --clear. Whatever named the old name is parted from it: edges and wires name a way out by name, so they are left pointing at a name nobody declares — visibly, in the picture, rather than silently rewritten. The error way out's name is fixed and refused at both ends.",
        ...
        cmd("automation exit rm", "Deletes a way out with the outputs declared on it. The error way out is refused — every step and every action carries one. The edges and wires that named it are left where they are, parted. Confirms unless --yes.",
            json!([{ "name": "<id>", "help": "way out id", "required": true },
                   { "name": "--yes/-y", "help": "skip the confirmation" }]),
        ...
        cmd("automation port add", "Declares a port. The direction is not asked for — it falls out of what the port hangs off: --step and --action declare what is taken in, --exit declares what that way out hands on. Neither is sayable on the other's owner, which is what lets a review step hand on a file only when it left through \"something to fix\". --kind says what it carries: value, file, task_take (the task the run is working) or task_make (a task the step created).",
            json!([{ "name": "--step <id>", "help": "the step that takes it in (one carrying its own prompt)" },
        ...
        cmd("automation port update", "Changes a port's name, what it carries, or whether it is required. Only the fields given change. Renaming parts every wire that named the old name, for the reason a way out's rename does.",
        ...
        cmd("automation cfg add", "Declares a setting on a step or a library action: its name, what kind of answer it takes, and whether it has to be answered. --options is the choice list, as a JSON array, and belongs to --kind choice alone. An action's row is the declaration by itself; the answer is written on each step that uses it (`automation cfg set`). A step that runs a library action declares nothing of its own — write it on the action.",
            json!([{ "name": "--step <id>", "help": "the step that declares it (one carrying its own prompt)" },
                   { "name": "--action <id>", "help": "the library action that declares it" },
            ...
            json!(["amenbo automation cfg add --action 7 --name queue --kind taskfilter --required",
                   "amenbo automation cfg add --step 12 --name depth --kind choice --options '[\"quick\",\"deep\"]'"])),
        ...
        cmd("automation cfg set", "Answers a setting on one step, by the name it was declared under. A step running a library action takes a row of its own under that name, so two steps running one action answer it differently. --clear leaves it unanswered. THE ANSWER IS WRITTEN IN THE SHAPE ITS KIND TAKES, NEVER AS ONE FILTER STRING: --folder / --choice / --number / --text each take one value, and a taskfilter is built from --status / --priority / --assignee / --dim / --ready / --done / --due — the same option twice is any-of, two different options are both, which is the reading filterGrammar gives. The expression that builds is parsed before the answer is written, so a value nothing accepts is refused here rather than at the launch of a run. Nothing reads the declaration on the way through (no command lists one), so an answer in the wrong shape for its kind is caught when the run reads it.",
            ...
            json!(["amenbo automation cfg set 12 --name queue --status todo --status in_progress --priority high",
                   "amenbo automation cfg set 12 --name depth --choice deep"])),
        cmd("automation cfg rm", "Deletes a setting — the declaration, and on a step the answer written on the same row. Confirms unless --yes.",
            json!([{ "name": "<id>", "help": "setting id", "required": true },
                   { "name": "--yes/-y", "help": "skip the confirmation" }]),
            json!(["amenbo automation cfg rm 41 --yes"])),

        cmd("automation edge add", "Says what happens after one step leaves through one way out: go on to another step (--to), close the run (--done), or stop it and call a person (--halt). The edge carries no condition of its own — the way out is the condition — and one way out says one thing, so a second edge on the same one is refused. --from is written as one token, <step>:<way out>: `4:` is the unnamed way out and `4:*` the error one, because a way out is named against whichever of the step and its action declares it. --max-times caps how often a --to edge may be taken FOR ONE TASK, counted afresh at the next task; left out it is 10, which guards a loop that never converges without stopping a review that goes round three times. --no-max takes the cap off. A --done or --halt edge is taken once and carries no cap at all.",
            json!([{ "name": "--from <step>:<way out>", "help": "where it leaves from (`4:` is the unnamed way out, `4:*` the error one)", "required": true },
                   { "name": "--to <id>", "help": "go on to this step" },
                   { "name": "--done", "help": "close the run" },
                   { "name": "--halt", "help": "stop the run and call a person" },
        ...
        cmd("automation edge update", "Changes where an edge goes, or how often it may be taken. Only the fields given change. The way out it hangs on is not among them — that pair is what the edge is, so pointing it at another way out is a delete and an add.",
            json!([{ "name": "<id>", "help": "edge id", "required": true },
                   { "name": "--to <id> / --done / --halt", "help": "what happens after that way out" },
        ...
        cmd("automation wire add", "Joins what one way out hands on to what a later step takes in. Both ends are named rather than keyed: the same action used at two places in one automation gives two steps whose ports carry the same names, so only the step, the way out and the port name together say which is meant. --from is the same <step>:<way out> token the edge takes. The two ends are refused unless they carry the same kind of thing.",
            json!([{ "name": "--from <step>:<way out>", "help": "where it comes from (`4:` is the unnamed way out)", "required": true },
                   { "name": "--from-port <name>", "help": "the output's name on that way out", "required": true },
                   { "name": "--to <id>", "help": "the step that takes it in", "required": true },
                   { "name": "--to-port <name>", "help": "the input's name on that step", "required": true }]),
            json!(["amenbo automation wire add --from \"11:something to fix\" --from-port report --to 12 --to-port report"])),
        cmd("automation wire rm", "Deletes a wire — the later step is then handed nothing under that name. Confirms unless --yes.",
            json!([{ "name": "<id>", "help": "wire id", "required": true },
                   { "name": "--yes/-y", "help": "skip the confirmation" }]),
            json!(["amenbo automation wire rm 61 --yes"])),

        cmd("automation note add", "Writes a document the steps of one automation share. Long is fine here — this is where the material a prompt would otherwise repeat belongs, which is what keeps the preamble short. Which steps are handed it is `automation note link`'s to say.",
            json!([{ "name": "<automation>", "help": "automation id", "required": true },
                   { "name": "--name <str>", "help": "what this document is called", "required": true },
                   { "name": "--body <text>", "help": "the document itself, in Markdown (`-` reads stdin)", "required": true }]),
            json!(["amenbo automation note add 3 --name \"House style\" --body -"])),
        cmd("automation note update", "Renames a shared document, or rewrites it. Only the fields given change.",
            json!([{ "name": "<id>", "help": "document id", "required": true },
                   { "name": "--name <str>", "help": "rename it" },
                   { "name": "--body <text>", "help": "rewrite it (`-` reads stdin)" }]),
            json!(["amenbo automation note update 71 --body -"])),
        cmd("automation note rm", "Deletes a shared document with the links that hand it to steps. Confirms unless --yes.",
            json!([{ "name": "<id>", "help": "document id", "required": true },
                   { "name": "--yes/-y", "help": "skip the confirmation" }]),
            json!(["amenbo automation note rm 71 --yes"])),
        cmd("automation note link", "Hands a shared document to a step: it is read out to that step alongside the preamble and the step's own prompt. A document may go to several steps, and a step may be handed several.",
            json!([{ "name": "<step>", "help": "step id", "required": true },
        ...
        cmd("automation note unlink", "Stops handing a shared document to a step. The document itself stays where it is.",
            json!([{ "name": "<step>", "help": "step id", "required": true },
                   { "name": "<note>", "help": "document id", "required": true }]),
            json!(["amenbo automation note unlink 11 71"])),
    ])
}
