...
# The one name the production CLI is bundled, installed and typed under — `Paths::command_name()`
...
# The names tauri itself has to be told, as one config object. Written once, here, because more than
# one build reads them: the mac bundle below, the Linux AppImage (gui-dev-linux), and the Windows
# installer the preview workflow builds on a runner that has no make. That last one is why
...
# theme.
#
# The bundled CLI (externalBin) is split by the same word as the app-data, and it has to be: the
# Windows installer puts it on PATH, so a channel shipping it as `amenbo.exe` puts production, the
# shared dev build and every theme on PATH under one name, and which one a shell resolves is decided
# by the order they were installed in. amenbo-core answers the same word (Paths::command_name), and
# the app-data name (GUI_DEV_DATA) reaches the build as AMENBO_APP_NAME in the environment — which is
# also what app/scripts/prepare-sidecars.mjs stages the file under, so the two cannot drift.
#
# The askpass helper git is pointed at is the second sidecar, and it splits the same way for the
# same reason: what the installer puts on PATH is the whole directory, not the one file anybody was
# told to type (amenbo-core, Paths::askpass_file_name).
GUI_DEV_CONFIG := {"productName":"$(GUI_DEV_NAME)","identifier":"$(GUI_DEV_ID)","mainBinaryName":"$(GUI_DEV_BIN)","bundle":{"externalBin":["binaries/$(GUI_DEV_DATA)","binaries/$(GUI_DEV_DATA)-askpass"]}}
...
help:
	...
	@echo "make skin-backgrounds-gate - assert every rule painting one of the four places a skin may lay a picture behind reads that place's picture slot (a rule that leaves it out keeps this build's flat colour while every other surface wears the author's) = the same guard CI runs (automatic at the start of make test)"
...
dist-gui:
	@mkdir -p $(DIST_DIR)
	@scripts/write-tick-plist.sh $(TICK_LABEL) $(CLI_NAME)
...
gate-cheap:
	...
	$(MAKE) --no-print-directory skin-backgrounds-gate
...
gate-rust:
	...
	## CI shards the e2e run across runners on top of the split, the other one being short enough to
	## run whole there as well; here both run whole, there being one machine either way.
...
gate-app-rust:
	## The app crate's build.rs (tauri_build) checks that every sidecar tauri.conf.json's externalBin
	## names exists (binaries/$(CLI_NAME) and its -askpass here — this stage builds no channel
	## override). Both are generated by tauri's beforeBuildCommand, but that does not run under a
	## direct cargo invocation, and binaries/ is gitignored, so a fresh worktree fails every time with
	## `resource path … doesn't exist`. Pre-generate them with the same script the GUI build uses.
...
## matches is one whose change SKIPS the job — green PR, and the parity break lands on main under an
## unrelated one. A skipped job is not a red one, so nothing else would say it.
## Declared once and shared: `make test` and CI's tree-guards both run this file.
gui-inputs-gate:
	@guards/check-gui-inputs.sh
...
## Guard the answer to "may a skin move this?", which is made once per token and compiled into the
## binary that applies it. A token added to tokens.css and named on neither list is settable by
## nobody and reported to nobody, and a name left on a list after its token is gone reads as
## coverage. Neither shows any other way.
## Declared once and shared: `make test` and CI's tree-guards both run this file.
skin-vocabulary-gate:
	@guards/check-skin-vocabulary.sh

## Guard the pairing that carries a skin's picture to a surface: a place is not an element, so the
## picture has to go behind every rule that paints in that colour, and the slot beside the colour is
## what carries it. A rule that paints the colour and leaves the slot out draws exactly as well as
## one that does not — nothing is undefined and no test has anything to compare against, and the
## card simply keeps this build's flat colour while every other one wears the author's paper.
## Declared once and shared: `make test` and CI's tree-guards both run this file.
skin-backgrounds-gate:
	@guards/check-skin-backgrounds.sh
...
## under app/src-tauri/ (which is what a bundler actually reads).
## Everything it writes is tracked, so this is not a build step and nothing waits on it — it is run
