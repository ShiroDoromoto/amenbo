...
export const ja: Translation = {
  ...
  ui: {
    ...
    "dimmgr.appliesToHint": "この分類をタスクと決定記録のどちらに出すか。分類そのものが持つ設定なので、全員の見え方が変わる。絞っても、すでに付いている値は消えない ── そこで意味を持たなくなるだけ",
    ...
    "dimmgr.slugPh": "キー",
    "dimmgr.slugHint": "Amenbo の外から名前として使えるキー。英小文字・数字・ハイフンで、先頭は英字。重複はできず、空にもできない。",
    ...
    "settings.tickOn": "オン",
    "settings.tickOff": "オフ",
    "settings.tickNote": "オンにすると、この端末のスケジューラが1時間に1度 Amenbo を起こし、期日の近いタスクを探して、有効にしているプラグインから知らせを届けます。オフにすると登録を取り去り、裏で動き続けるものは残りません。",
    ...
    "settings.dataNote": "データはお使いの端末にローカル保存され、独自バイナリに閉じ込めません。エクスポートは全データ（全プロジェクト）をフォルダひとつに書き出します（他ツールへの移行用・片道）——可搬な JSON（export.json）と、添付ファイルそのもの（attachments/・付いていたタスクや決定ごとに並びます）。Amenbo へ戻すのはバックアップからの復元です。",
    ...
    "settings.backupNote": "この端末のデータ（全プロジェクト）を、添付ファイルそのものも含めて検証済みの単一ファイルへ書き出します（鍵は含まれません）。復元は破壊的ですが、直前の状態を自動退避します。端末外（iCloud 等）に保存する場合、ファイルは平文です——保存先の信頼は利用者の責任です（クラウド側の暗号とアカウント認証に委ねます）。",
