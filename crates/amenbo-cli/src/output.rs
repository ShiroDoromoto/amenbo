    ...
    fn from(e: amenbo_core::Error) -> CliError {
        ...
        let hint = match &e {
            ...
            E::NotReady(m) if m.code() == Some(ErrorCode::NotReadyAutomation) => Some(format!(
                "Every reason is named above, and each is answered by writing on the automation: `{cmd} automation step-add`, `automation entry-set`, `automation edge-add`, `automation wire-add`, `automation cfg-set`, `automation agent-set`."
            )),
