...
fn all_commands() -> Value {
    json!([
        ...
        cmd("automation builtin-list", "Amenbo's built-ins: steps it carries out itself — no terminal, no agent, no prompt. Each with its key, what it does, the settings it reads, what it takes in and the ways out it leaves by, with what each hands on. They are defined in Amenbo and cannot be edited; `automation place-add --builtin` and `automation step-add --builtin` put one on a picture.",
            json!([{ "name": "--json", "help": "machine-readable output" }]),
        ...
        cmd("automation step-add", "Adds a step to a library action — one step is one terminal, and it carries its own prompt. Who carries it out is not the step's: it is chosen where the action is placed (`automation agent-set`), so the same action can be run by different agents on two automations. It is born with the unnamed way out and the error one (`*`). --work-dir names the setting or the input the working folder is taken from: a name, not a path, and the name is one the action declares. --builtin puts in one of Amenbo's built-ins instead: a step Amenbo carries out itself, with no terminal and no prompt, whose ways out and ports come from its definition and cannot be edited; the settings it reads are declared on the action. Prints the id the action's own edge and wire commands take.",
            json!([{ "name": "<action>", "help": "action id", "required": true },
                   { "name": "--name <str>", "help": "what this step is called (unless --builtin)" },
                   { "name": "--prompt <text>", "help": "the prompt this step runs on (`-` reads stdin; unless --builtin)" },
                   { "name": "--builtin <key>", "help": "put in a built-in (`automation builtin-list`)" },
                   ...
                   { "name": "--no-task-context", "help": "do not hand it the task the run is on — its notes, linked decisions and comments" }]),
        ...
        cmd("automation step-update", "Changes a step. Only the fields given change. Rewriting the prompt reaches every placement of the action holding it, which is what the library is for — and is refused while a run placing that action is running or paused.",
            json!([{ "name": "<id>", "help": "step id", "required": true },
                   ...
                   { "name": "--task-context <true|false>", "help": "whether it is handed the task the run is on — its notes, linked decisions and comments" }]),
