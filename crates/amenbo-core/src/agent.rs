...
const JA_PHRASEBOOK: &[(&str, &str)] = &[
    ...
    ("See this machine's installed plugins, open or close each one's gate (install ≠ enable, machine-wide or for this project), and remove one with everything it left behind", "このマシンにインストール済みのプラグインを見て、それぞれのゲートを（マシン全体、またはこのプロジェクトだけで）開閉し（install ≠ enable）、残したものごと撤去する"),
...
fn capabilities() -> Value {
    let caps = vec![
        ...
        cap(
            "See this machine's installed plugins, open or close each one's gate (install ≠ enable, machine-wide or for this project), and remove one with everything it left behind",
            &["plugin list", "plugin enable", "plugin disable", "plugin inherit", "plugin uninstall"],
        ),
        cap(
