...
const JA_PHRASEBOOK: &[(&str, &str)] = &[
    ...
    ("Lists decisions (status:proposed|accepted|rejected, current:yes|no — being superseded is not a status but a derived view, so current:no lists the decisions another decision supersedes — text: over title+body+comment bodies, project:, decided_before:/decided_after: over the day a decision was accepted (YYYY-MM-DD, or today/-30d; both ends inclusive; a decision that was never accepted has no such day and matches neither)). To ask which policies were settled by a date, compose this filter with current: — there is no separate as-of switch, and the composition recovers neither status transitions nor deleted decisions. Sort by decided/created/number/title/status (prefix - for descending; default -created). --limit/--offset page (JSON carries total_matched = the count before paging, count = this page). --with-body adds each decision's body to the rows — a projection that composes with --filter/--limit/--offset (narrow by keywords/status and page; it does not dump the whole corpus).", "決定記録を一覧します（status:proposed|accepted|rejected、current:yes|no ── 「置換された」は status ではなく派生ビューです。current:no ＝ 別の決定に置き換えられた決定、text: はタイトル＋本文＋コメント本文が対象、project:、decided_before:/decided_after: は採択された日で絞ります〔YYYY-MM-DD または today/-30d・両端を含む。採択されていない決定はその日を持たず、どちらにも一致しません〕）。「T までに決まっていた方針」は、このフィルタと current: の合成で得ます ── 専用の as-of スイッチはありません。合成でもステータス遷移の履歴や削除された決定は辿れません。並び替えは decided/created/number/title/status（先頭 - で降順・既定 -created）。--limit/--offset はページングします（JSON は total_matched = ページング前の件数、count = このページの件数を持ちます）。--with-body は各決定の本文を行に足します ── --filter/--limit/--offset と合成できる射影です（キーワード/状態で絞ってページングする。コーパス全体を吐き出しはしません）。"),
    ...
    ("the task comment ref to promote, AMB-TC-n", "昇格するタスクコメント参照 AMB-TC-n"),
    ...
    ("Validates a plugin manifest file against the catalog rules — a well-formed id, repo, non-empty OS set and config schema, plus a distributable in one of the two forms an entry may take (one https url and checksum for every OS it lists, or one per OS, whose platforms must be exactly the ones declared) — reporting every problem it finds so an author can self-check before opening a catalog PR. It reads the same rules amenbo enforces at the install/intake door, so the two never disagree. The path may be .yaml (the form authored in the catalog repo) or .json (the aggregated catalog.json form); the format is taken from the extension, defaulting to YAML. A manifest that does not even parse is reported too — a missing required field is the shape half of the fail-closed door. It opens no store and needs no binding, so it runs anywhere (a plugin checkout, CI). On --json a passing manifest also carries the manifest amenbo read (a 'manifest' field, the whole shape), so a consumer such as the catalog aggregator builds install entries from that reading rather than its own list of which fields to copy — a list that silently drops a field amenbo later adds; a manifest that does not pass carries no body. The exit code is the verdict: 0 valid, 1 invalid (or the file could not be read).", "プラグインの manifest ファイルを目録の規約——整った id・repo・空でない対応OS集合・config スキーマ、および2つの形式のいずれかによる配布物（挙げた全 OS に配る1つの https url と checksum、または OS ごとに1つずつで、その OS 集合は宣言と一致すること）——に照らして検証し、見つかった問題をすべて報告します。作者は目録へ PR を出す前に自己チェックできます。amenbo が install/取り込みの入口で強制するのと同じ規約を読むため、両者が食い違うことはありません。パスは .yaml（目録リポで作者が書く形式）でも .json（集約後の catalog.json 形式）でもよく、形式は拡張子で判定し、既定は YAML です。そもそもパースできない manifest も報告します——必須フィールドの欠落は fail-closed の入口の「形」のチェックで弾かれます。ストアを開かず binding も不要なので、どこでも（プラグインのチェックアウトや CI）走ります。--json では検証を通った manifest は amenbo が読み取った manifest 本体（'manifest' フィールド・形すべて）も併せて返すので、目録の集約側などの利用者は、写す項目を自前で並べたリスト——amenbo が後から項目を足しても黙って落ちるリスト——ではなく、その読み取りからインストールエントリを組み立てられます。通らなかった manifest は本体を返しません。終了コードが判定です: 0 は有効、1 は無効（またはファイルを読めなかった）。"),
    ...
    ("Installs a plugin from the catalog: resolves the name against the catalog amenbo holds (fetched fresh when the network answers, the cached copy when it does not), downloads the asset its manifest points at, verifies it fail-closed, and lays it down under the app-data plugins/ directory. Verification is the whole point of the door: the asset's minisign signature against the catalog public key amenbo ships (origin), then the manifest's checksum over the exact bytes served (integrity) — unsigned, signed by any other key, or a digest that does not match, and nothing is written. Installing never enables: the plugin lands inert and `plugin enable` is the separate, explicit act, which is also where compatibility with this build is judged. A name already installed is refused rather than overwritten, and so is a broken install in the way (uninstall it first) — a home left by an install that did not finish is not one, so a retry goes straight through. An OS the manifest does not list is refused too — a platform the entry never claimed has no build behind it — and the asset fetched is the one published for the OS running the install, since an entry may carry a separate distributable per platform. The asset may be a gzip'd tar holding an entry named after the plugin, or the executable itself; a zip is refused by name. The only command in this group that touches the network.", "目録からプラグインをインストールします：amenbo が持つ目録に名前を照会し（ネットワークが応じれば取り直した最新、応じなければ手元のキャッシュ）、manifest が指すアセットを取得し、fail-closed で検証してから、app-data の plugins/ ディレクトリに設置します。この入口の要点は検証そのものです：アセットの minisign 署名を amenbo が同梱する目録公開鍵で検証し（出所）、次に manifest のチェックサムを実際に配られたバイト列に対して照合します（完全性）。未署名・別の鍵による署名・ダイジェスト不一致のいずれでも、何も書きません。インストールは有効化ではありません：プラグインは不活性のまま置かれ、`plugin enable` が別の明示的な手順です（この版との互換もそこで判定します）。すでにインストール済みの名前は上書きせず拒否し、壊れたインストールが残っている場合も拒否します（先に uninstall してください）——途中で終わった install が残した置き場はインストールではないので、やり直しはそのまま通ります。manifest が挙げていない OS も拒否します——目録が名乗っていない環境には、そもそも作られたビルドがありません——そして取得するアセットは、install を走らせている OS 向けに公開されたものです（エントリは OS ごとに別の配布物を持てます）。アセットは、プラグイン名のエントリを含む gzip 圧縮 tar か、実行ファイルそのものです。zip はその旨を告げて拒否します。このグループでネットワークに触れる唯一のコマンドです。"),
    ("the plugin's name, as the catalog lists it", "目録に載っているプラグインの名前"),
    ("Enables an installed plugin: records the one-time consent to run its code, and opens the one gate it fires through. Which gate that is was declared by the plugin's author, not chosen here: a plugin declaring scope: project turns on for the project you are in (so it needs a bound folder, and turning it on elsewhere is a separate act), one declaring scope: machine turns on for the device. That is why there is no --scope — a plugin has one switch, and a user is never shown two; the reply names the level it moved. Installing puts a plugin on disk and nothing more — this is the step that lets it run, and it is asked once: a later disable keeps the consent, so re-enabling never asks again, and the consent is the device's whichever gate was opened (an enable row carried to a machine that never consented fires nothing). Fail-closed on the settings the plugin's author marked required — while one is empty the enable is refused and the empty fields are named; fill them with `plugin config set` and enable again. amenbo checks only that a value is present, at the tier that gate reads (a project's enable counts that project's overrides on top of the machine defaults); whether the value is *meaningful* is the plugin author's to judge at run time. Fail-closed on compatibility too: a plugin whose manifest reads a different event-payload contract than this amenbo speaks, or needs an amenbo newer than the one running, is refused with both versions named — update amenbo (or the plugin) rather than run one against a payload it cannot read.", "インストール済みのプラグインを有効化します：そのコードを実行することへの一度きりの同意を記録し、発火する唯一のゲートを開きます。どのゲートかを決めるのはここではなく作者の宣言です：`scope: project` のプラグインは今いるプロジェクトで on になり（バインド済みフォルダが要ります。別のプロジェクトで on にするのは別の操作です）、`scope: machine` のものは端末で on になります。だから --scope はありません——プラグインのスイッチは1つで、利用者に2つ見せることはありません。応答はどの層を動かしたかを名乗ります。インストールはプラグインをディスクに置くだけで、実行を許すのはこの手順です。同意を尋ねるのは一度だけ——後で disable しても同意は残るので、再度の enable で尋ね直しません。同意はどちらのゲートを開いた場合でも端末のものです（有効化の行を、同意していない端末へ持ち込んでも何も発火しません）。作者が required と印した設定に対しては fail-closed です——1つでも空なら enable は拒否され、空のフィールド名が示されます。`plugin config set` で埋めてから enable し直してください。amenbo が見るのは値が入っているかどうかだけで、見る層はそのゲートが読む層です（プロジェクトの有効化なら、マシン既定の上にそのプロジェクトの上書きも数えます）。値が意味として妥当かどうかは、実行時に作者が判断することです。互換宣言に対しても fail-closed です：manifest が読むイベントペイロード規約がこの amenbo の話す規約と違う場合、または必要とする amenbo の版が実行中の版より新しい場合、双方の版を示して拒否します——読めないペイロードを渡して走らせるのではなく、amenbo（またはプラグイン）を更新してください。"),
    ("the installed plugin's name", "インストール済みプラグインの名前"),
    ("Closes a plugin's gate — the same single switch `enable` opens, at the level its author declared, so there is no --scope here either. It stops firing, while staying installed and staying consented, so enabling it again later asks for nothing. Deliberately does not require the plugin to still read as installed: this is how a plugin is stopped, and a half-broken install is exactly when stopping it matters most — with no readable manifest there is no declaration left to follow, so every gate the name could hold is closed. Disabling one that is already off changes nothing and says so.", "プラグインのゲートを閉じます——`enable` が開くのと同じ唯一のスイッチを、作者が宣言した層で閉じるので、ここにも --scope はありません。発火は止まりますが、インストール済みのまま・同意も保持したままなので、後で有効化し直すときに何も尋ねられません。プラグインがインストール済みとして読めることを、あえて要求しません：これはプラグインを止める手段であり、壊れかけたインストールこそ止めたい場面だからです——manifest が読めなければ従うべき宣言も無いので、その名前が持ちうるゲートをすべて閉じます。すでに無効なものを disable しても何も変わらず、その旨を伝えます。"),
    ...
    // plugin runs (the execution log, read back — the answer to "my plugin did nothing")
    ...
    ("Stores one of an installed plugin's settings. The key must be one the plugin's manifest declares — that declaration is also what says whether the value is a secret, and amenbo never judges that for itself: a secret goes to the user-area secret file (off the store, off every backup, injected later as an environment variable), everything else to the ordinary tiers. --scope picks the tier: machine (the default, in config.json) or project (this project's override); which project is never named here, it is the folder's binding (a human may move that with the global --project). A secret ignores the tiers — there is one per plugin and key. Passing `-` as the value reads it from stdin, which is how a token stays off argv and out of shell history; the trailing newline a pipe adds is dropped, and nothing else. An empty value clears the setting rather than storing a blank, so this is also the unset door. The value is never echoed back. Filling the fields the author marked required is what lets `plugin enable` through.", "インストール済みプラグインの設定を1つ保存します。キーはそのプラグインの manifest が宣言しているものでなければなりません——その宣言こそが「値が秘密かどうか」を告げるもので、amenbo が自分で判断することは決してありません：秘密はユーザー領域の秘密ファイルへ（ストアの外・全バックアップの外。実行時には環境変数として注入されます）、それ以外は通常の層へ入ります。--scope が層を選びます：machine（既定・config.json）か project（このプロジェクトの上書き）。どのプロジェクトかをここで名指すことはなく、フォルダの binding が決めます（人間は全体フラグの --project で動かせます）。秘密は層を無視します——プラグインとキーの組ごとに1つです。値に `-` を渡すと標準入力から読みます。トークンを argv とシェル履歴に残さないための経路で、パイプが付ける末尾の改行だけを落とし、それ以外には手を触れません。空の値は空白を保存するのではなく設定を消すので、これは解除の入口でもあります。値がエコーで返ることはありません。作者が required と印したフィールドを埋めることが、`plugin enable` を通す条件です。"),
...
fn capabilities() -> Value {
    let caps = vec![
        ...
        cap(
            "Put a plugin from the catalog on this machine, see what is installed and what the catalog has moved past, bring one onto the published build or roll that back, open or close each one's gate (install ≠ enable, machine-wide or for this project), and remove one with everything it left behind",
            &[
                "plugin list",
                "plugin install",
                "plugin update",
...
fn all_commands() -> Value {
    json!([
        ...
        json!({ "name": "decision promote", "summary": "Promotes a task comment into a decision: the comment text becomes the body, the project defaults to the comment's task project, and the new decision is linked to that task.",
            "args": [{ "name": "comment", "required": true, "help": "the task comment ref to promote, AMB-TC-n" }],
            ...
            "examples": ["amenbo decision promote AMB-TC-<n> --title \"Standardize on ISO-8601 dates\""] }),
        ...
        json!({ "name": "plugin runs", "summary": "Reads the plugin execution log: the last runs of each plugin, newest first, narrowed to one when you name it. A hook is fire-and-forget — nobody waits on it, and nothing fails when it fails — so this is the only place that answers 'my plugin did nothing, why'. One line per run: when it ran, which plugin, on which event, how it ended (ok / failed / timed_out / not_launched), its exit code and how long it took. A run that did not end cleanly is followed by what the plugin wrote to stderr, which is where its author put the diagnosis; --json carries that text for every run, clean ones included. A gap line is not a run at all — it marks events that reached nobody because retention trimmed them away before the dispatcher read them, and it names no plugin because what was lost was never resolved to one. A name with nothing on file reports an empty log rather than an error. Reads one machine-local file and nothing else: no store rows, no network, and nothing that leaves this device (the log is outside every backup and export). It is bounded by construction — the last runs of each installed plugin, each with a capped slice of stderr — so there is no window to ask for and no deeper history to page: a longer one is a logging plugin's business, not amenbo's. No secret can appear in it, structurally: the log is never handed a plugin's environment, so there is no field one could ride in.",
            "args": [{ "name": "name", "required": false, "help": "narrow to one plugin's runs; omit for every plugin's, newest first" }],
            "flags": [{ "name": "--json", "help": "machine-readable output" }],
            "examples": ["amenbo plugin runs", "amenbo plugin runs slack --json"] }),
        json!({ "name": "plugin install", "summary": "Installs a plugin from the catalog: resolves the name against the catalog amenbo holds (fetched fresh when the network answers, the cached copy when it does not), downloads the asset its manifest points at, verifies it fail-closed, and lays it down under the app-data plugins/ directory. Verification is the whole point of the door: the asset's minisign signature against the catalog public key amenbo ships (origin), then the manifest's checksum over the exact bytes served (integrity) — unsigned, signed by any other key, or a digest that does not match, and nothing is written. Installing never enables: the plugin lands inert and `plugin enable` is the separate, explicit act, which is also where compatibility with this build is judged. A name already installed is refused rather than overwritten, and so is a broken install in the way (uninstall it first) — a home left by an install that did not finish is not one, so a retry goes straight through. An OS the manifest does not list is refused too — a platform the entry never claimed has no build behind it — and the asset fetched is the one published for the OS running the install, since an entry may carry a separate distributable per platform. The asset may be a gzip'd tar holding an entry named after the plugin, or the executable itself; a zip is refused by name. The only command in this group that touches the network.",
        ...
        json!({ "name": "plugin disable", "summary": "Closes a plugin's gate — the same single switch `enable` opens, at the level its author declared, so there is no --scope here either. It stops firing, while staying installed and staying consented, so enabling it again later asks for nothing. Deliberately does not require the plugin to still read as installed: this is how a plugin is stopped, and a half-broken install is exactly when stopping it matters most — with no readable manifest there is no declaration left to follow, so every gate the name could hold is closed. Disabling one that is already off changes nothing and says so.",
        ...
        json!({ "name": "plugin config set", "summary": "Stores one of an installed plugin's settings. The key must be one the plugin's manifest declares — that declaration is also what says whether the value is a secret, and amenbo never judges that for itself: a secret goes to the user-area secret file (off the store, off every backup, injected later as an environment variable), everything else to the ordinary tiers. --scope picks the tier: machine (the default, in config.json) or project (this project's override); which project is never named here, it is the folder's binding (a human may move that with the global --project). A secret ignores the tiers — there is one per plugin and key. Passing `-` as the value reads it from stdin, which is how a token stays off argv and out of shell history; the trailing newline a pipe adds is dropped, and nothing else. An empty value clears the setting rather than storing a blank, so this is also the unset door. The value is never echoed back. Filling the fields the author marked required is what lets `plugin enable` through.",
            ...
            "flags": [{ "name": "--scope <machine|project>", "help": "which tier to write (default machine)" }, { "name": "--json", "help": "machine-readable output" }],
            "examples": ["amenbo plugin config set slack events push,merge", "printf %s \"$TOKEN\" | amenbo plugin config set slack webhook_url -", "amenbo plugin config set slack events \"\" --scope project"] }),
        json!({ "name": "plugin config get", "summary": "Reads one of an installed plugin's settings back at the tier --scope names — the machine default or this project's override exactly as stored, not the effective value precedence would pick at run time. A secret's value never comes out this door, --json included: it reports only whether one is set, because a get that prints a token puts it in the terminal, the scrollback and the shell's history. Injection reads secrets whole, into the plugin's environment and nowhere else. A key the manifest does not declare is refused with the keys it does declare, so a typo answers with the vocabulary rather than a silent 'not set'.",
