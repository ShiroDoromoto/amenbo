...
# also what app/scripts/prepare-cli-sidecar.mjs stages the file under, so the two cannot drift.
GUI_DEV_CONFIG := {"productName":"$(GUI_DEV_NAME)","identifier":"$(GUI_DEV_ID)","mainBinaryName":"$(GUI_DEV_BIN)","bundle":{"externalBin":["binaries/$(GUI_DEV_DATA)"]}}
...
gate-app-rust:
	## The app crate's build.rs (tauri_build) checks that the sidecar tauri.conf.json's externalBin
	## names exists (binaries/$(CLI_NAME) here — this stage builds no channel override). The sidecar
	## CLI is generated by tauri's beforeBuildCommand, but that does not run under a
